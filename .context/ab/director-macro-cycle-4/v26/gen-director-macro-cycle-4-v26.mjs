...
const directorMacroCycle4V26Only = process.env.BUILD_DIRECTOR_MACRO_CYCLE_4_V26 === '1';
const directorMacroCycle4V25Only = process.env.BUILD_DIRECTOR_MACRO_CYCLE_4_V25 === '1' || directorMacroCycle4V26Only;
const directorMacroCycle4V24Only = process.env.BUILD_DIRECTOR_MACRO_CYCLE_4_V24 === '1' || directorMacroCycle4V25Only;
const directorMacroCycle4Only = process.env.BUILD_DIRECTOR_MACRO_CYCLE_4 === '1' || directorMacroCycle4V24Only;
const directorMacroCycle3Only = process.env.BUILD_DIRECTOR_MACRO_CYCLE_3 === '1' || directorMacroCycle4Only;
const directorMacroCycle2Only = process.env.BUILD_DIRECTOR_MACRO_CYCLE_2 === '1' || directorMacroCycle3Only;
...
const v24FeatureGroups = [];
const v25OperationalDetailGroups = [];
...
const M = {
  ...
  campusPaving: mat(0x3f4746, { roughness: 0.94, metalness: 0.04 }),
...
const edgeBox = (w, h, d, x, y, z, m, ry = 0, radius = 0.06) => {
  ...
  const g = new Mesh(new RoundedBoxGeometry(w, h, d, 1, r), m);
...
if (directorMacroCycle4Only) {
  const campusHardstand = box(930, 2.2, 670, 0, baseTileY, 10, M.campusPaving);
  group.add(campusHardstand);
  const sectors = [
    [-382, 272, 142, 82, M.loadingAsphaltRough], [-238, 282, 92, 58, M.hardstand],
    [-100, 292, 132, 64, M.loadingAsphalt], [34, 284, 118, 58, M.hardstand],
    [-394, 86, 102, 92, M.loadingAsphalt], [-376, -82, 106, 92, M.hardstand],
    [-312, -218, 132, 68, M.loadingAsphaltRough], [-112, -286, 132, 52, M.hardstand],
    [54, -288, 128, 48, M.loadingAsphalt],
    [280, 240, 64, 48, M.hardstand], [382, 250, 84, 70, M.loadingAsphalt],
    [326, 168, 72, 50, M.hardstand], [418, 170, 78, 70, M.loadingAsphaltRough],
    [340, -28, 96, 80, M.loadingAsphalt], [420, -150, 82, 68, M.hardstand],
    [382, -245, 104, 72, M.loadingAsphaltRough], [300, -280, 112, 62, M.loadingAsphalt],
    [300, 90, 66, 48, M.loadingAsphalt], [380, 80, 58, 42, M.loadingAsphaltRough],
  ];
  for (const [x, z, width, depth, material] of sectors) {
    const patch = flatPolygon(x, z, width, depth, material, 0, baseTileY + 1.18, idSeed(`cycle4-sector-${x}-${z}`), 9);
    patch.userData.worldUvPeriod = 12;
    group.add(patch);
    group.add(flatSegment([x - width * 0.38, baseTileY + 1.2, z - depth * 0.32], [x + width * 0.38, baseTileY + 1.2, z - depth * 0.32], 0.16, M.roadJoint, 0.01));
    group.add(flatSegment([x - width * 0.38, baseTileY + 1.2, z + depth * 0.32], [x + width * 0.38, baseTileY + 1.2, z + depth * 0.32], 0.12, M.curb, 0.01));
  }
  const perimeterLoop = [[-430, 292], [-430, -286], [112, -286], [142, -218], [142, 286], [-430, 292]];
  const perimeterRoad = ribbon(perimeterLoop.map(([x, z]) => [x, baseTileY + 1.23, z]), 8.5, M.wetRoad, { topOffset: 0.02, thickness: 0.08 });
  perimeterRoad.userData.worldUvPeriod = 8.4;
  group.add(perimeterRoad);
  for (let i = 0; i < perimeterLoop.length - 1; i++) {
    const [ax, az] = perimeterLoop[i], [bx, bz] = perimeterLoop[i + 1];
    group.add(flatSegment([ax, baseTileY + 1.34, az], [bx, baseTileY + 1.34, bz], 0.12, M.line, 0.01));
  }
  const westServiceLoop = [
    [-450, baseTileY + 1.25, -286], [-450, baseTileY + 1.25, 280],
    [-450, baseTileY + 1.25, -180], [-338, baseTileY + 1.25, -180],
    [-450, baseTileY + 1.25, -5], [-338, baseTileY + 1.25, -5],
  ];
  group.add(ribbon(westServiceLoop.slice(0, 2), 7.2, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  for (const pair of [[2, 3], [4, 5]]) group.add(ribbon(westServiceLoop.slice(pair[0], pair[1] + 1), 6.8, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  for (const z of [-260, -180, -95, -5, 85, 175, 260]) {
    group.add(flatSegment([-450, baseTileY + 1.36, z], [-446, baseTileY + 1.36, z], 0.12, M.line, 0.01));
    yardLampPole(-442, z, baseTileY + 1.2, 6.0, 0.9);
  }
  // The east service spine ties the outboard process yards back to the perimeter loop.
  const eastServiceX = 454;
  const eastSpine = ribbon([[eastServiceX, baseTileY + 1.25, 286], [eastServiceX, baseTileY + 1.25, -286]], 7.4, M.wetRoad, { topOffset: 0.02, thickness: 0.08 });
  eastSpine.userData.worldUvPeriod = 8.4;
  group.add(eastSpine);
  group.add(ribbon([[142, baseTileY + 1.25, 286], [eastServiceX, baseTileY + 1.25, 286]], 7.4, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  for (const [z, endX] of [[250, 416], [115, 443], [-30, 428], [-150, 435], [-245, 445], [-286, 345]]) {
    group.add(ribbon([[eastServiceX, baseTileY + 1.25, z], [endX, baseTileY + 1.25, z]], 6.8, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
    group.add(flatSegment([eastServiceX, baseTileY + 1.36, z], [endX, baseTileY + 1.36, z], 0.11, M.line, 0.01));
    yardLampPole(eastServiceX - 6, z, baseTileY + 1.2, 6.0, 0.9);
  }
  for (const z of [-260, -180, -95, -5, 85, 175, 260]) {
    group.add(flatSegment([eastServiceX, baseTileY + 1.36, z], [eastServiceX, baseTileY + 1.36, z + 4], 0.11, M.line, 0.01));
    yardLampPole(eastServiceX - 4.5, z, baseTileY + 1.2, 6.0, 0.9);
  }
  const eastBoulevard = ribbon([[60, baseTileY + 1.25, 120], [eastServiceX, baseTileY + 1.25, 120]], 7.2, M.wetRoad, { topOffset: 0.02, thickness: 0.08 });
  eastBoulevard.userData.worldUvPeriod = 8.4;
  group.add(eastBoulevard);
  group.add(ribbon([[220, baseTileY + 1.25, 120], [220, baseTileY + 1.25, 286]], 6.6, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  for (const [ax, az, bx, bz] of [[60, 120, 454, 120], [220, 120, 220, 286]]) {
    group.add(flatSegment([ax, baseTileY + 1.36, az], [bx, baseTileY + 1.36, bz], 0.11, M.line, 0.01));
  }
  for (const [x, z] of [[112, 120], [190, 120], [220, 154], [220, 230], [300, 120], [380, 120]]) yardLampPole(x, z, baseTileY + 1.2, 6.0, 0.9);
  group.add(ribbon([[eastServiceX, baseTileY + 1.25, -205], [394, baseTileY + 1.25, -205]], 6.6, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  group.add(ribbon([[220, baseTileY + 1.25, -270], [300, baseTileY + 1.25, -285]], 6.6, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  group.add(flatSegment([eastServiceX, baseTileY + 1.36, -205], [394, baseTileY + 1.36, -205], 0.11, M.line, 0.01));
  group.add(flatSegment([220, baseTileY + 1.36, -270], [300, baseTileY + 1.36, -285], 0.11, M.line, 0.01));
  group.add(ribbon([[300, baseTileY + 1.25, 120], [300, baseTileY + 1.25, 114]], 6.4, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  group.add(flatSegment([300, baseTileY + 1.36, 120], [300, baseTileY + 1.36, 114], 0.11, M.line, 0.01));
  group.add(ribbon([[380, baseTileY + 1.25, 120], [380, baseTileY + 1.25, 108]], 6.2, M.wetRoad, { topOffset: 0.02, thickness: 0.08 }));
  group.add(flatSegment([380, baseTileY + 1.36, 120], [380, baseTileY + 1.36, 108], 0.11, M.line, 0.01));
  for (const [x, z] of [[420, -205], [380, -205], [250, -276]]) yardLampPole(x, z, baseTileY + 1.2, 6.0, 0.9);
} else for (let ix = 0; ix < 10; ix++) for (let iz = 0; iz < 10; iz++) {
...
if (!directorMacroCycle4Only) for (let ix = 1; ix < 10; ix++) {
...
if (!directorMacroCycle4Only) for (let iz = 1; iz < 10; iz++) {
...
// Director macro cycle 2: distributed facility families replace the central-only
// open-cell read with broad, grounded masses on the authored outboard pads.
const directorMacroCycle2SourceRefs = [
  'blockout/blockout-full-v1.json#segments.g-spawn-apron',
  'blockout/blockout-full-v1.json#segments.g-gate-square',
  'blockout/blockout-full-v1.json#segments.g-ridgeline-exit',
...
const directorMacroCycle2ReferenceIds = [
  'industrial-frontage', 'industrial-yard-operations', 'processing-yard',
  'service-access', 'industrial-macro-massing', 'facility-edge-vegetation',
];
const directorMacroCycle2ContractIds = [
  'source-canonical', 'routes-unchanged', 'route-clearance-2m',
  'air-lane-clearance', 'authored-structure-clearance', 'gameplay-space-clearance',
  'ground-contact', 'canonical-evidence', 'triangle-growth-15-percent',
  'distributed-family-massing',
];
const directorMacroCycle2Network = {
  schemaVersion: 1,
  strategy: 'distributed-facility-families-campus-cycle-2',
  owner: 'director-macro-cycle-2',
  sourceRefs: [...directorMacroCycle2SourceRefs],
  referenceIds: [...directorMacroCycle2ReferenceIds],
  contractIds: [...directorMacroCycle2ContractIds],
  evidenceViews: [
    'top', 'orbit', 'zone-spawn', 'zone-courtyard', 'zone-warehouse', 'zone-plant',
    'zone-core', 'zone-bridge', 'zone-boundary', 'route-main-surface', 'route-covered',
    'route-north-ring', 'route-south-retreat', 'objective-core', 'vertical-connector',
  ],
  clearancePolicy: { padInsetMeters: 2, routeClearanceMeters: 2, airLaneClearanceMeters: 2, buildingClearanceMeters: 2, gameplayClearanceMeters: 2 },
  compoundPolicy: { geometry: 'distributed-low-poly-facility-families', target: 'campus-wide-visible-massing', maxTriangles: 881216 },
  features: [],
};
const directorMacroCycle2Specs = [
  { id: 'c2-gate-admin', padId: 'g-gate-square', center: [-335, 128], width: 18, depth: 12, height: 10, family: 'admin', zone: 'zone_courtyard', routes: ['route_main_surface', 'route_rear_alley', 'air-courtyard'], pairedWith: 'c2-spawn-logistics', relationship: 'arrival-admin-to-logistics', evidenceViews: ['top', 'orbit', 'zone-courtyard', 'zone-spawn', 'route-main-surface', 'route-rear-alley'] },
  { id: 'c2-spawn-logistics', padId: 'g-spawn-apron', center: [-335, 270], width: 24, depth: 16, height: 7, family: 'loading', zone: 'zone_spawn', routes: ['route_ridgeline_exit', 'air-spawn-apron'], pairedWith: 'c2-ridgeline-boundary', relationship: 'arrival-yard-to-perimeter-service', evidenceViews: ['top', 'orbit', 'zone-spawn', 'route-main-surface', 'zone-boundary', 'route-ridgeline-exit'] },
  { id: 'c2-ridgeline-boundary', padId: 'g-ridgeline-exit', center: [-215, 323], width: 24, depth: 10, height: 5.5, family: 'boundary', zone: 'zone_boundary', routes: ['route_ridgeline_exit', 'air-spawn-apron'], pairedWith: 'c2-spawn-logistics', relationship: 'perimeter-service-to-arrival-yard', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-spawn', 'route-ridgeline-exit', 'route-main-surface'] },
  { id: 'c2-west-warehouse', padId: 'g-yrd-west', center: [-235, 30], width: 34, depth: 26, height: 12, family: 'warehouse', zone: 'zone_warehouse', routes: ['route_maintenance_loop', 'route_covered'], pairedWith: 'c2-core-admin', relationship: 'maintenance-warehouse-to-core-admin', evidenceViews: ['top', 'orbit', 'zone-warehouse', 'route-covered', 'vertical-connector', 'zone-courtyard'] },
  { id: 'c2-pressure-process', padId: 'g-yrd-pressure', center: [180, 175], width: 30, depth: 18, height: 15, family: 'process', zone: 'zone_courtyard', routes: ['route_main_surface', 'route_north_ring', 'air-pressure-yard'], pairedWith: 'c2-east-process', relationship: 'pressure-yard-to-process-transfer', evidenceViews: ['top', 'orbit', 'zone-courtyard', 'zone-plant', 'route-main-surface', 'route-north-ring'] },
  { id: 'c2-core-admin', padId: 'g-yrd-core-n', center: [-20, -195], width: 28, depth: 14, height: 15, family: 'core', zone: 'zone_core', routes: ['route_main_surface', 'route_covered', 'route_tunnel'], pairedWith: 'c2-west-warehouse', relationship: 'core-admin-to-maintenance-warehouse', evidenceViews: ['top', 'orbit', 'zone-core', 'objective-core', 'route-covered', 'tunnel-portal'] },
  { id: 'c2-east-process', padId: 'g-yrd-e', center: [360, -85], width: 26, depth: 26, height: 20, family: 'plant', zone: 'zone_plant', routes: ['route_north_ring', 'route_south_retreat', 'air-roof-reentry'], pairedWith: 'c2-plant-yard', relationship: 'process-tower-to-loading-yard', evidenceViews: ['top', 'orbit', 'zone-plant', 'route-north-ring', 'route-south-retreat', 'vertical-connector'] },
  { id: 'c2-plant-yard', padId: 'g-yrd-plant-s', center: [350, -150], width: 28, depth: 12, height: 8, family: 'loading', zone: 'zone_plant', routes: ['route_south_retreat', 'route_plant_spur', 'air-roof-reentry'], pairedWith: 'c2-east-ridge-service', relationship: 'plant-loading-to-tunnel-service', evidenceViews: ['top', 'orbit', 'zone-plant', 'route-south-retreat', 'route-north-ring', 'zone-boundary'] },
  { id: 'c2-east-ridge-service', padId: 'g-yrd-east-ridge', center: [350, -240], width: 36, depth: 22, height: 10, family: 'service', zone: 'zone_tunnels', routes: ['route_tunnel', 'route_flank_backdoor', 'route_south_retreat'], pairedWith: 'c2-plant-yard', relationship: 'tunnel-service-to-plant-loading', evidenceViews: ['top', 'orbit', 'zone-tunnels', 'tunnel-portal', 'route-flank-backdoor', 'zone-boundary'] },
];
const directorMacroCycle2Bounds = spec => ({
  minX: spec.center[0] - spec.width / 2,
  minZ: spec.center[1] - spec.depth / 2,
  maxX: spec.center[0] + spec.width / 2,
  maxZ: spec.center[1] + spec.depth / 2,
  width: spec.width,
  depth: spec.depth,
});
const directorMacroCycle2BoxDistance = (a, c) => Math.hypot(Math.max(c.minX - a.maxX, 0, a.minX - c.maxX), Math.max(c.minZ - a.maxZ, 0, a.minZ - c.maxZ));
const directorMacroCycle2RouteDistance = (x, z, route) => {
  let best = Infinity;
  for (let i = 0; i < route.waypoints.length - 1; i++) best = Math.min(best, yardOpsDistanceToSegment(x, z, route.waypoints[i][0], route.waypoints[i][1], route.waypoints[i + 1][0], route.waypoints[i + 1][1]));
  return best;
};
const directorMacroCycle2RouteMargin = (bounds, route) => directorMacroCycle2RouteDistance((bounds.minX + bounds.maxX) / 2, (bounds.minZ + bounds.maxZ) / 2, route)
  - (route.width || 6) / 2 - Math.hypot(bounds.width / 2, bounds.depth / 2);
const directorMacroCycle2Placement = spec => {
  const bounds = directorMacroCycle2Bounds(spec), pad = yardOpsPads.get(spec.padId);
  const padBounds = pad && { minX: Math.min(pad.bounds[0], pad.bounds[2]), minZ: Math.min(pad.bounds[1], pad.bounds[3]), maxX: Math.max(pad.bounds[0], pad.bounds[2]), maxZ: Math.max(pad.bounds[1], pad.bounds[3]) };
  const padPass = Boolean(padBounds && bounds.minX >= padBounds.minX + 2 && bounds.maxX <= padBounds.maxX - 2 && bounds.minZ >= padBounds.minZ + 2 && bounds.maxZ <= padBounds.maxZ - 2);
  const buildingMargins = yardOpsBuildings.map(building => directorMacroCycle2BoxDistance(bounds, { minX: Math.min(building.bounds[0], building.bounds[2]), minZ: Math.min(building.bounds[1], building.bounds[3]), maxX: Math.max(building.bounds[0], building.bounds[2]), maxZ: Math.max(building.bounds[1], building.bounds[3]) }));
  const routeMargins = b.routes.filter(route => route.kind !== 'air').map(route => ({ id: route.id, margin: directorMacroCycle2RouteMargin(bounds, route) }));
  const airMargins = b.routes.filter(route => route.kind === 'air').map(route => ({ id: route.id, margin: directorMacroCycle2RouteMargin(bounds, route) }));
  const gameplayMargins = segs.filter(segment => ['kill-zone', 'objective', 'cover', 'entrance-player', 'stair', 'incline', 'bridge', 'hole-drone-entry'].includes(segment.category)).map(segment => ({ id: segment.id, margin: directorMacroCycle2BoxDistance(bounds, { minX: Math.min(segment.bounds[0], segment.bounds[2]), minZ: Math.min(segment.bounds[1], segment.bounds[3]), maxX: Math.max(segment.bounds[0], segment.bounds[2]), maxZ: Math.max(segment.bounds[1], segment.bounds[3]) }) }));
  const pairMargins = directorMacroCycle2Network.features.map(feature => ({ id: feature.id, margin: directorMacroCycle2BoxDistance(bounds, directorMacroCycle2Bounds({ center: feature.center, width: feature.footprint[0], depth: feature.footprint[1] })) }));
  const padClear = Boolean(pad && yardOpsClear(pad, spec.center[0], spec.center[1], bounds.width, bounds.depth, 2));
  const pass = padPass && padClear && buildingMargins.every(margin => margin >= 2) && routeMargins.every(item => item.margin >= 2)
    && airMargins.every(item => item.margin >= 2) && gameplayMargins.every(item => item.margin >= 2) && pairMargins.every(item => item.margin >= 2);
  return { bounds, padPass: padPass && padClear, buildingMargins, routeMargins, airMargins, gameplayMargins, pairMargins, pass };
};
const directorMacroCycle2Apron = (spec, sy) => {
  const { center: [x, z], width, depth } = spec;
  const apron = box(width * 0.96, 0.08, depth * 0.94, x, sy + 0.04, z, M.loadingWet);
  apron.userData.worldUvPeriod = 8.0;
  group.add(apron);
  for (const side of [-1, 1]) group.add(box(width * 0.9, 0.13, 0.2, x, sy + 0.12, z + side * depth * 0.43, M.curb));
  for (const side of [-1, 1]) group.add(box(0.2, 0.13, depth * 0.86, x + side * width * 0.43, sy + 0.12, z, M.curb));
  for (const lx of [-width * 0.34, -width * 0.12, width * 0.12, width * 0.34]) {
    group.add(box(0.18, 0.035, 3.8, x + lx, sy + 0.18, z - depth * 0.33, M.line));
    group.add(box(0.18, 0.035, 2.2, x + lx, sy + 0.18, z + depth * 0.3, M.lineWhite));
  }
  group.add(flatPolygon(x - width * 0.23, z + depth * 0.25, width * 0.18, depth * 0.16, M.loadingPuddle, 0, sy + 0.17, idSeed(`${spec.id}-wet-1`), 7));
  group.add(flatPolygon(x + width * 0.22, z - depth * 0.2, width * 0.12, depth * 0.12, M.loadingStain, 0, sy + 0.17, idSeed(`${spec.id}-wet-2`), 7));
  for (const lx of [-width * 0.32, width * 0.32]) group.add(box(1.0, 0.05, 0.16, x + lx, sy + 0.2, z + depth * 0.39, M.drain));
};
const directorMacroCycle2Shell = (x, z, width, depth, sy, height, material, rotation = 0) => {
  group.add(edgeBox(width, height, depth, x, sy + height / 2, z, material, rotation, 0.12));
  group.add(edgeBox(width + 0.5, 0.24, depth + 0.5, x, sy + height + 0.12, z, M.roof, rotation, 0.04));
  const local = (lx, lz) => rotateLocal(x, z, rotation, lx, lz);
  for (const lx of [-width * 0.36, -width * 0.12, width * 0.12, width * 0.36]) {
    for (const lz of [-depth / 2 - 0.08, depth / 2 + 0.08]) {
      const [px, pz] = local(lx, lz);
      group.add(edgeBox(0.16, Math.max(2.2, height - 0.35), 0.2, px, sy + height / 2, pz, M.rib, rotation, 0.02));
    }
  }
  const frontZ = -depth / 2 - 0.12;
  for (const lx of [-width * 0.27, 0, width * 0.27]) {
    const [px, pz] = local(lx, frontZ);
    group.add(box(Math.min(4.8, width * 0.18), Math.min(1.8, height * 0.2), 0.08, px, sy + Math.min(4.1, height * 0.58), pz, M.heroGlass, rotation));
  }
  return { local, frontZ };
};
const directorMacroCycle2LoadingBays = (x, z, sy, width, depth, height, rotation = 0) => {
  const local = (lx, lz) => rotateLocal(x, z, rotation, lx, lz), frontZ = -depth / 2 - 0.16;
  for (const lx of [-width * 0.28, 0, width * 0.28]) {
    const [px, pz] = local(lx, frontZ);
    group.add(edgeBox(Math.min(4.8, width * 0.23), Math.min(3.2, height * 0.62), 0.14, px, sy + Math.min(3.2, height * 0.62) / 2, pz, M.loadingDoor, rotation, 0.025));
    group.add(edgeBox(Math.min(5.2, width * 0.26), 0.18, 0.2, px, sy + Math.min(3.2, height * 0.62) + 0.25, pz, M.loadingFrame, rotation, 0.025));
  }
  const canopyZ = frontZ - 1.2;
  group.add(edgeBox(width * 0.84, 0.18, 2.2, x, sy + Math.min(4.1, height * 0.64), z + canopyZ, M.loadingCanopy, rotation, 0.04));
  for (const lx of [-width * 0.38, width * 0.38]) {
    const [px, pz] = local(lx, canopyZ);
    group.add(box(0.18, Math.min(4.0, height * 0.62), 0.18, px, sy + Math.min(4.0, height * 0.62) / 2, pz, M.loadingFrame, rotation));
  }
};
const directorMacroCycle2Stairs = (x, z, sy, width, run, rise, rotation = 0) => {
  const local = (lx, lz) => rotateLocal(x, z, rotation, lx, lz), steps = 7;
  for (let i = 0; i < steps; i++) {
    const [px, pz] = local(0, (i - steps / 2 + 0.5) * run / steps);
    group.add(box(width, rise * (i + 1), run / steps + 0.05, px, sy + rise * (i + 1) / 2, pz, M.concreteDark, rotation));
  }
  const [aX, aZ] = local(-width / 2, -run / 2), [bX, bZ] = local(-width / 2, run / 2);
  const [cX, cZ] = local(width / 2, -run / 2), [dX, dZ] = local(width / 2, run / 2);
  group.add(addBeam([aX, sy + 0.7, aZ], [bX, sy + rise * steps + 0.7, bZ], 0.06, M.fence));
  group.add(addBeam([cX, sy + 0.7, cZ], [dX, sy + rise * steps + 0.7, dZ], 0.06, M.fence));
};
const directorMacroCycle2Tree = (x, z, sy, scale = 1) => {
  group.add(taperedCylinder(0.13 * scale, 0.28 * scale, 1.9 * scale, x, sy + 0.95 * scale, z, M.rust, 7));
  group.add(taperedCylinder(0.08 * scale, 0.92 * scale, 1.6 * scale, x, sy + 2.35 * scale, z, M.campusBaseAlt, 7));
  group.add(taperedCylinder(0.04 * scale, 0.62 * scale, 1.2 * scale, x + 0.18 * scale, sy + 3.45 * scale, z, M.campusBase, 7));
};
const directorMacroCycle2FeatureGeometry = (spec, sy) => {
  const [x, z] = spec.center;
  directorMacroCycle2Apron(spec, sy);
  if (spec.family === 'warehouse') {
    directorMacroCycle2Shell(x, z + 2, 25, 17, sy, spec.height, M.heroSiding);
    directorMacroCycle2LoadingBays(x, z - 5.5, sy, 21, 11, spec.height);
    pipeGallery(x - 13, z - 6, x - 13, z + 8, sy, 5.1, 2.8);
    cargoTruck(x + 6, z - 8, 0, 0.68); forklift(x - 7, z - 8, 0, 0.68, sy); palletStack(x + 8, z + 7, sy, 2.2, 1.4, 2);
    chainLinkFence(x - 15, z - 10, x - 15, z + 9, sy, 2.4);
  } else if (spec.family === 'process') {
    directorMacroCycle2Shell(x - 3, z + 1, 19, 12, sy, 9.5, M.heroPanel);
    directorMacroCycle2LoadingBays(x - 3, z - 4, sy, 15, 7, 9.5);
    addSilo(x + 7, z - 5, sy, 2.4, 10, M.siding[1]); addSilo(x + 11, z + 2, sy, 1.8, 7, M.siding[2]);
    pipeRack(x - 12, z - 5, x + 13, z - 5, sy, 6.4, 3.2);
    serviceStack(x - 10, z + 5, sy, 8.5, 0.24); processVessel(x + 9, z + 7, sy, 1.35, 6.5, M.heroSidingAlt);
    fenceRun(x - 13, z - 8, x + 13, z - 8, sy, 2.7); forklift(x + 2, z - 7, 0.1, 0.62, sy);
  } else if (spec.family === 'plant') {
    directorMacroCycle2Shell(x, z + 1, 17, 18, sy, 13.5, M.siding[1]);
    const towerX = x + 7, towerZ = z - 3;
    group.add(edgeBox(6.2, spec.height, 6.2, towerX, sy + spec.height / 2, towerZ, M.heroPanel, 0, 0.09));
    group.add(edgeBox(6.6, 0.22, 6.6, towerX, sy + spec.height + 0.12, towerZ, M.roof, 0, 0.04));
    for (const y of [sy + 4, sy + 8, sy + 12, sy + 16]) group.add(box(7.1, 0.14, 0.18, towerX, y, towerZ - 3.2, M.rib));
    addSilo(x - 7, z - 5, sy, 2.5, 12, M.siding[0]); addSilo(x - 7, z + 5, sy, 1.7, 8, M.siding[2]);
    pipeGallery(x - 11, z - 8, x + 10, z - 8, sy, 8.2, 4.0); serviceStack(x + 8, z + 7, sy, 10.5, 0.25);
    directorMacroCycle2Stairs(x - 9, z + 5, sy, 2.4, 5.2, 0.26, 0); fenceRun(x - 12, z - 10, x + 12, z - 10, sy, 2.8);
    group.add(addBeam([x + 7, sy + 8.2, z - 8], [x + 7, sy + 8.2, z + 5], 0.17, M.pipe));
  } else if (spec.family === 'admin' || spec.family === 'core') {
    const mainH = spec.family === 'core' ? 12.5 : 8.2;
    directorMacroCycle2Shell(x - 1, z + 1, 13, 9, sy, mainH, M.concrete[1]);
    directorMacroCycle2Shell(x + 6, z + 3, 7, 6, sy, spec.family === 'core' ? 8 : 5.2, M.heroPanel);
    group.add(edgeBox(11.5, 0.22, 2.4, x, sy + mainH * 0.58, z - 5.0, M.heroGlass, 0, 0.035));
    directorMacroCycle2Stairs(x - 6, z - 4.8, sy, 2.4, 4.4, 0.24, 0);
    fenceRun(x - 10, z + 7, x + 10, z + 7, sy, 2.2); serviceCabinet(x + 9, z - 5, sy, 1.4, 2.0);
    group.add(box(5.2, 0.18, 2.2, x + 4, sy + mainH + 0.7, z + 1, M.roof));
  } else if (spec.family === 'loading') {
    directorMacroCycle2Shell(x, z + 1, spec.width * 0.62, spec.depth * 0.55, sy, spec.height * 0.7, M.panel[2]);
    directorMacroCycle2LoadingBays(x, z - 2.6, sy, spec.width * 0.54, spec.depth * 0.42, spec.height * 0.7);
    cargoTruck(x + spec.width * 0.24, z - spec.depth * 0.28, 0, 0.58); forklift(x - spec.width * 0.25, z - spec.depth * 0.28, 0, 0.58, sy);
    palletStack(x + spec.width * 0.2, z + spec.depth * 0.28, sy, 1.8, 1.2, 2); cableReel(x - spec.width * 0.24, z + spec.depth * 0.27, sy, 0.58);
    chainLinkFence(x - spec.width * 0.44, z - spec.depth * 0.4, x - spec.width * 0.44, z + spec.depth * 0.4, sy, 2.5);
  } else if (spec.family === 'service') {
    directorMacroCycle2Shell(x, z + 1, 24, 13, sy, 7.5, M.panel[1]);
    pipeGallery(x - 9, z - 5, x + 10, z - 5, sy, 5.8, 3.6); serviceStack(x + 8, z + 5, sy, 7.4, 0.2);
    directorMacroCycle2LoadingBays(x, z - 4, sy, 18, 7, 7.5); serviceCabinet(x - 9, z + 5, sy, 1.5, 2.0);
    chainLinkFence(x - 15, z - 9, x + 15, z - 9, sy, 2.6); directorMacroCycle2Stairs(x + 10, z + 4.5, sy, 2.2, 4.2, 0.24, 0);
    const frame = segmentFrame(x - 12, z + 7, x + 12, z + 7);
    openCellTunnelArchRib(x, z + 7, frame, sy - 1.5, 7, 5.2, M.tunnelConcreteDark);
  } else if (spec.family === 'boundary') {
    directorMacroCycle2Shell(x, z, 16, 6, sy, 4.6, M.panel[0]);
    pipeRack(x - 6, z - 1.5, x + 6, z - 1.5, sy, 4.0, 2.4); serviceStack(x + 5, z + 1.8, sy, 5.2, 0.18);
    chainLinkFence(x - 11, z - 4, x + 11, z - 4, sy, 3.0); chainLinkFence(x + 11, z - 4, x + 11, z + 4, sy, 3.0);
    serviceCabinet(x - 6, z + 1.8, sy, 1.4, 1.8); directorMacroCycle2Tree(x - 10, z + 4.2, sy, 0.9); directorMacroCycle2Tree(x + 9, z + 4.2, sy, 0.75);
  }
};
const directorMacroCycle2AddFeature = spec => {
  const placement = directorMacroCycle2Placement(spec);
  const feature = {
    id: spec.id, owner: spec.id, type: `distributed-${spec.family}-facility`, zone: spec.zone, padId: spec.padId,
    center: [...spec.center], footprint: [spec.width, spec.depth], height: spec.height, family: spec.family, routes: [...spec.routes],
    pairedWith: spec.pairedWith, relationship: spec.relationship, sourceRefs: [...directorMacroCycle2SourceRefs],
    referenceIds: [...directorMacroCycle2ReferenceIds], contractIds: [...directorMacroCycle2ContractIds], evidenceViews: [...spec.evidenceViews],
    routeClearanceMeters: 2, buildingClearanceMeters: 2, airLaneClearanceMeters: 2, gameplayClearanceMeters: 2,
    placementStatus: placement.pass ? 'PASS' : 'FAIL', supportStatus: placement.pass ? 'PASS' : 'FAIL', contactStatus: placement.pass ? 'PASS' : 'FAIL',
    clearance: {
      pad: placement.padPass ? 'PASS' : 'FAIL',
      minimumBuildingMargin: Math.min(...placement.buildingMargins),
  ...
  directorMacroCycle2Network.features.push(feature);
  if (!directorMacroCycle2Only || !placement.pass) return;
  const pad = yardOpsPads.get(spec.padId), sy = surfaceYAt(spec.center[0], spec.center[1], pad?.surfaceY ?? 0) + 0.06;
  directorMacroCycle2FeatureGeometry(spec, sy);
};
for (const spec of directorMacroCycle2Specs) directorMacroCycle2AddFeature(spec);
if (directorMacroCycle2Only) {
  // A high pipe transfer physically binds the east process tower to the lower loading yard.
  pipeGallery(360, -103, 350, -137, 7.9, 7.2, 3.8);
  fenceRun(337, -137, 363, -137, -1, 2.5);
  for (const [x, z, scale] of [[374, -120, 0.8], [328, -126, 0.7], [374, -205, 0.85], [328, -205, 0.7], [-350, 292, 0.75], [-315, 292, 0.9], [-270, 338, 0.8], [-188, 338, 0.72]]) {
    directorMacroCycle2Tree(x, z, surfaceYAt(x, z, 0), scale);
  }
}

// Director macro cycle 3: connect the outboard families into darker, shared yards
// rather than adding another scatter of isolated boxes.
const directorMacroCycle3SourceRefs = [
  ...directorMacroCycle2SourceRefs,
  'blockout/blockout-full-v1.json#segments.g-yrd-rear',
  'blockout/blockout-full-v1.json#segments.g-yrd-hub',
  'blockout/blockout-full-v1.json#segments.g-mix-courts',
];
const directorMacroCycle3ReferenceIds = [
  ...directorMacroCycle2ReferenceIds,
  'user-supplied-industrial-campus-board',
];
const directorMacroCycle3ContractIds = [
  ...directorMacroCycle2ContractIds,
  'connected-compound-massing',
  'shared-operational-yards',
];
const directorMacroCycle3Network = {
  schemaVersion: 1,
  strategy: 'connected-dark-compounds-campus-cycle-3',
  owner: 'director-macro-cycle-3',
  sourceRefs: [...directorMacroCycle3SourceRefs],
  referenceIds: [...directorMacroCycle3ReferenceIds],
  contractIds: [...directorMacroCycle3ContractIds],
  evidenceViews: ['top', 'orbit', 'zone-courtyard', 'zone-warehouse', 'zone-plant', 'zone-core', 'zone-boundary', 'route-main-surface', 'route-north-ring', 'route-south-retreat', 'objective-core', 'vertical-connector'],
  compoundPolicy: { geometry: 'connected-low-poly-industrial-compounds', target: 'campus-wide-visible-massing', maxTriangles: 881216 },
  features: [],
};
const directorMacroCycle3Specs = [
  { id: 'c3-gate-yard', padId: 'g-gate-square', center: [-335, 110], width: 36, depth: 16, height: 10, family: 'admin', zone: 'zone_courtyard', routes: ['route_main_surface', 'route_rear_alley', 'air-courtyard'], pairedWith: 'c3-west-north', relationship: 'controlled-gate-to-warehouse-yard' },
  { id: 'c3-west-north', padId: 'g-yrd-west', center: [-240, 70], width: 32, depth: 20, height: 12, family: 'warehouse', zone: 'zone_warehouse', routes: ['route_maintenance_loop', 'route_covered'], pairedWith: 'c3-west-south', relationship: 'warehouse-to-service-yard' },
  { id: 'c3-west-south', padId: 'g-yrd-west', center: [-225, -90], width: 54, depth: 30, height: 11, family: 'service', zone: 'zone_warehouse', routes: ['route_maintenance_loop', 'route_covered'], pairedWith: 'c3-west-north', relationship: 'service-yard-to-warehouse' },
  { id: 'c3-plant-yard', padId: 'g-yrd-plant-s', center: [300, -145], width: 44, depth: 18, height: 16, family: 'process', zone: 'zone_plant', routes: ['route_south_retreat', 'route_plant_spur'], pairedWith: 'c3-east-service', relationship: 'process-yard-to-tunnel-service' },
  { id: 'c3-east-service', padId: 'g-yrd-east-ridge', center: [245, -248], width: 50, depth: 16, height: 10, family: 'loading', zone: 'zone_tunnels', routes: ['route_tunnel', 'route_flank_backdoor', 'route_south_retreat'], pairedWith: 'c3-plant-yard', relationship: 'tunnel-service-to-process-yard' },
  { id: 'c3-core-yard', padId: 'g-yrd-core-n', center: [-60, -185], width: 30, depth: 14, height: 15, family: 'core', zone: 'zone_core', routes: ['route_covered', 'route_tunnel'], pairedWith: 'c3-plant-yard', relationship: 'core-service-to-process-yard' },
];
const directorMacroCycle3FeatureGeometry = (spec, sy) => {
  const [x, z] = spec.center;
  directorMacroCycle2Apron(spec, sy);
  const material = spec.family === 'process' ? M.heroPanel : spec.family === 'core' ? M.facade[1] : M.heroSidingAlt;
  const shellWidth = Math.min(spec.width * 0.74, 42);
  const shellDepth = Math.min(spec.depth * 0.7, 22);
  const shellHeight = Math.min(spec.height, 14);
  directorMacroCycle2Shell(x, z + 2, shellWidth, shellDepth, sy, shellHeight, material);
  directorMacroCycle2LoadingBays(x, z - shellDepth * 0.48, sy, shellWidth * 0.88, 8, shellHeight);
  for (const lx of [-shellWidth * 0.34, shellWidth * 0.34]) {
    const [px, pz] = rotateLocal(x, z, 0, lx, shellDepth * 0.55);
    group.add(edgeBox(4.0, 0.16, 0.18, px, sy + 0.45, pz, M.concreteDark, 0, 0.02));
  }
  if (spec.family === 'warehouse' || spec.family === 'service') {
    const annexX = x + (spec.family === 'warehouse' ? shellWidth * 0.52 : -shellWidth * 0.5);
    const annexZ = z + shellDepth * 0.72;
    group.add(edgeBox(shellWidth * 0.38, shellHeight * 0.56, shellDepth * 0.38, annexX, sy + shellHeight * 0.28, annexZ, M.panel[1], 0, 0.05));
    group.add(edgeBox(shellWidth * 0.41, 0.16, shellDepth * 0.41, annexX, sy + shellHeight * 0.56 + 0.1, annexZ, M.roof, 0, 0.03));
    pipeGallery(x - shellWidth * 0.46, z + shellDepth * 0.62, annexX, annexZ, sy + Math.min(7.5, shellHeight * 0.58), 5.2, 2.6);
    fenceRun(x - shellWidth * 0.52, z - shellDepth * 0.64, x + shellWidth * 0.52, z - shellDepth * 0.64, sy, 2.6);
    palletStack(x + shellWidth * 0.3, z + shellDepth * 0.62, sy, 1.8, 1.15, 2);
  } else if (spec.family === 'process') {
    addSilo(x + shellWidth * 0.36, z - shellDepth * 0.44, sy, 2.2, 10, M.siding[1]);
    addSilo(x + shellWidth * 0.48, z + shellDepth * 0.1, sy, 1.5, 7, M.siding[2]);
    pipeRack(x - shellWidth * 0.48, z - shellDepth * 0.62, x + shellWidth * 0.52, z - shellDepth * 0.62, sy, 7.0, 3.2);
    directorMacroCycle2Stairs(x - shellWidth * 0.36, z + shellDepth * 0.56, sy, 2.2, 4.6, 0.24);
    fenceRun(x - shellWidth * 0.56, z - shellDepth * 0.68, x + shellWidth * 0.56, z - shellDepth * 0.68, sy, 2.8);
  } else if (spec.family === 'core') {
    directorMacroCycle2Shell(x + shellWidth * 0.3, z + shellDepth * 0.18, shellWidth * 0.38, shellDepth * 0.62, sy, shellHeight * 0.7, M.concreteDark);
    directorMacroCycle2Stairs(x - shellWidth * 0.34, z - shellDepth * 0.56, sy, 2.4, 5.2, 0.25);
    group.add(edgeBox(shellWidth * 0.9, 0.18, 2.0, x, sy + shellHeight * 0.62, z - shellDepth * 0.56, M.heroGlass, 0, 0.03));
  } else {
    cargoTruck(x + shellWidth * 0.3, z - shellDepth * 0.65, 0, 0.62);
    forklift(x - shellWidth * 0.28, z - shellDepth * 0.65, 0, 0.58, sy);
    chainLinkFence(x - shellWidth * 0.56, z + shellDepth * 0.62, x + shellWidth * 0.56, z + shellDepth * 0.62, sy, 2.5);
  }
};
const directorMacroCycle3AddFeature = spec => {
  const placement = directorMacroCycle2Placement(spec);
  const feature = {
    id: spec.id, owner: spec.id, type: `connected-${spec.family}-compound`, zone: spec.zone, padId: spec.padId,
    center: [...spec.center], footprint: [spec.width, spec.depth], height: spec.height, family: spec.family, routes: [...spec.routes],
    pairedWith: spec.pairedWith, relationship: spec.relationship, sourceRefs: [...directorMacroCycle3SourceRefs],
    referenceIds: [...directorMacroCycle3ReferenceIds], contractIds: [...directorMacroCycle3ContractIds], evidenceViews: [...directorMacroCycle3Network.evidenceViews],
    routeClearanceMeters: 2, buildingClearanceMeters: 2, airLaneClearanceMeters: 2, gameplayClearanceMeters: 2,
    placementStatus: placement.pass ? 'PASS' : 'FAIL', supportStatus: placement.pass ? 'PASS' : 'FAIL', contactStatus: placement.pass ? 'PASS' : 'FAIL',
    clearance: { pad: placement.padPass ? 'PASS' : 'FAIL', minimumBuildingMargin: Math.min(...placement.buildingMargins), minimumRouteMargin: Math.min(...placement.routeMargins.map(item => item.margin)), minimumAirLaneMargin: Math.min(...placement.airMargins.map(item => item.margin)), minimumGameplayMargin: Math.min(...placement.gameplayMargins.map(item => item.margin)) },
  };
  directorMacroCycle3Network.features.push(feature);
  if (placement.pass) {
    const pad = yardOpsPads.get(spec.padId), sy = surfaceYAt(spec.center[0], spec.center[1], pad?.surfaceY ?? 0) + 0.06;
    directorMacroCycle3FeatureGeometry(spec, sy);
  }
};
if (directorMacroCycle3Only) {
  for (const spec of directorMacroCycle3Specs) directorMacroCycle3AddFeature(spec);
  // High service bridges make the outboard pads read as one working campus.
  pipeGallery(-240, 59, -225, -75, 9.4, 6.4, 2.8);
  pipeGallery(300, -136, 245, -240, 10.6, 6.2, 3.0);
  flatPolygon(145, 174, 82, 30, M.loadingWet, 0, surfaceYAt(145, 174, 0) + 0.12, idSeed('c3-pressure-shared-yard'), 8);
  flatPolygon(-145, 82, 72, 36, M.loadingAsphalt, 0, surfaceYAt(-145, 82, 0) + 0.12, idSeed('c3-west-shared-yard'), 8);
  for (const [x, z, scale] of [[-260, 98, 0.8], [-118, 100, 0.7], [-245, -92, 0.75], [238, -132, 0.8], [315, -180, 0.75], [340, -275, 0.8], [114, -260, 0.72]]) directorMacroCycle2Tree(x, z, surfaceYAt(x, z, 0), scale);
}

// Director macro cycle 4: replace the checkerboard read with a continuous
// industrial hardstand and occupy only the safe outboard context bands.
const directorMacroCycle4SourceRefs = [
  ...directorMacroCycle3SourceRefs,
  'blockout/blockout-full-v1.json#terrain.defaultSurfaceY',
  'blockout/blockout-full-v1.json#segments.g-spawn-apron',
  'blockout/blockout-full-v1.json#segments.g-ridgeline-exit',
];
const directorMacroCycle4ReferenceIds = [
  ...directorMacroCycle3ReferenceIds,
  'wet-industrial-hardstand', 'perimeter-service-context', 'layered-campus-silhouettes',
];
const directorMacroCycle4ContractIds = [
  ...directorMacroCycle3ContractIds,
  'continuous-campus-substrate', 'outboard-context-clearance', 'facility-edge-transition',
];
const directorMacroCycle4Network = {
  schemaVersion: 1,
  strategy: 'continuous-hardstand-outboard-context-campus-cycle-4',
  owner: 'director-macro-cycle-4',
  sourceRefs: [...directorMacroCycle4SourceRefs],
  referenceIds: [...directorMacroCycle4ReferenceIds],
  contractIds: [...directorMacroCycle4ContractIds],
  evidenceViews: ['top', 'orbit', 'zone-spawn', 'zone-boundary', 'zone-warehouse', 'zone-core', 'route-main-surface', 'route-rear-alley', 'route-south-retreat', 'vertical-connector', 'tunnel-portal', 'objective-core'],
  clearancePolicy: { routeClearanceMeters: 2, airLaneClearanceMeters: 2, buildingClearanceMeters: 2 },
  compoundPolicy: { geometry: 'continuous-industrial-hardstand-with-outboard-service-bands', target: 'campus-wide-visible-massing', maxTriangles: 881216 },
  features: [],
};
const directorMacroCycle4Specs = [
  { id: 'c4-north-utility-belt', center: [-86, 318], width: 116, depth: 16, height: 7, kind: 'north-rack', pairedWith: 'c4-south-logistics-belt', relationship: 'north-utility-to-south-logistics', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-spawn', 'route-main-surface', 'vertical-connector'] },
  { id: 'c4-west-edge-service', center: [-412, 78], width: 18, depth: 150, height: 6, kind: 'west-service', pairedWith: 'c4-west-utility-yard', relationship: 'perimeter-service-to-utility-yard', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-warehouse', 'route-main-surface', 'route-rear-alley'] },
  { id: 'c4-south-logistics-belt', center: [-170, -300], width: 190, depth: 16, height: 6, kind: 'south-loading', pairedWith: 'c4-north-utility-belt', relationship: 'south-logistics-to-north-utility', evidenceViews: ['top', 'orbit', 'zone-core', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
  { id: 'c4-west-utility-yard', center: [-380, -155], width: 42, depth: 34, height: 8, kind: 'west-utility', pairedWith: 'c4-west-edge-service', relationship: 'utility-yard-to-perimeter-service', evidenceViews: ['top', 'orbit', 'zone-warehouse', 'zone-boundary', 'route-rear-alley', 'objective-core'] },
  { id: 'c4-west-south-yard', center: [-390, -220], width: 110, depth: 50, height: 8, kind: 'edge-yard', pairedWith: 'c4-west-mid-yard', relationship: 'southwest-logistics-to-west-maintenance', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-warehouse', 'route-rear-alley', 'route-south-retreat'] },
  { id: 'c4-west-mid-yard', center: [-400, -40], width: 80, depth: 50, height: 9, kind: 'edge-yard', pairedWith: 'c4-west-south-yard', relationship: 'west-maintenance-to-southwest-logistics', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-warehouse', 'route-main-surface', 'route-rear-alley'] },
  { id: 'c4-south-west-yard', center: [-330, -270], width: 130, depth: 50, height: 7, kind: 'edge-yard', pairedWith: 'c4-south-left-yard', relationship: 'south-staging-to-south-service', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-core', 'route-south-retreat', 'vertical-connector'] },
  { id: 'c4-south-left-yard', center: [-420, -120], width: 50, depth: 60, height: 7, kind: 'edge-yard', pairedWith: 'c4-south-west-yard', relationship: 'south-service-to-south-staging', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-warehouse', 'route-rear-alley', 'objective-core'] },
  { id: 'c4-east-shore-yard', center: [100, 240], width: 72, depth: 42, height: 8, kind: 'edge-yard', pairedWith: 'c4-west-utility-yard', relationship: 'shore-service-to-west-utility', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-plant', 'route-north-ring', 'vertical-connector'] },
  { id: 'c4-west-lower-hub', center: [-300, -205], width: 60, depth: 34, height: 10, kind: 'edge-yard', pairedWith: 'c4-west-south-yard', relationship: 'lower-west-hub-to-southwest-logistics', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-warehouse', 'route-rear-alley', 'route-south-retreat'] },
  { id: 'c4-west-center-hub', center: [-300, -20], width: 60, depth: 34, height: 10, kind: 'edge-yard', pairedWith: 'c4-west-lower-hub', relationship: 'center-west-hub-to-lower-west-hub', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-warehouse', 'route-main-surface', 'objective-core'] },
  { id: 'c4-east-north-yard', center: [380, 250], width: 72, depth: 60, height: 14, kind: 'east-yard', pairedWith: 'c4-east-mid-yard', relationship: 'north-east-storage-to-east-service', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-plant', 'route-north-ring', 'vertical-connector'] },
  { id: 'c4-east-mid-yard', center: [408, 170], width: 70, depth: 62, height: 14, kind: 'east-yard', pairedWith: 'c4-east-north-yard', relationship: 'east-service-to-north-east-storage', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-plant', 'route-north-ring', 'route-south-retreat'] },
  { id: 'c4-east-process-yard', center: [388, -30], width: 80, depth: 64, height: 15, kind: 'process-yard', pairedWith: 'c4-east-ridge-yard', relationship: 'east-process-to-ridge-transfer', evidenceViews: ['top', 'orbit', 'zone-plant', 'route-north-ring', 'route-south-retreat', 'vertical-connector'] },
  { id: 'c4-east-ridge-yard', center: [400, -150], width: 70, depth: 58, height: 14, kind: 'east-yard', pairedWith: 'c4-east-process-yard', relationship: 'ridge-transfer-to-east-process', evidenceViews: ['top', 'orbit', 'zone-plant', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
  { id: 'c4-east-tank-yard', center: [410, -245], width: 70, depth: 48, height: 12, kind: 'process-yard', pairedWith: 'c4-south-east-loading', relationship: 'tank-transfer-to-south-loading', evidenceViews: ['top', 'orbit', 'zone-plant', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
  { id: 'c4-south-east-loading', center: [300, -285], width: 90, depth: 50, height: 9, kind: 'south-loading', pairedWith: 'c4-east-tank-yard', relationship: 'south-loading-to-tank-transfer', evidenceViews: ['top', 'orbit', 'zone-core', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
  { id: 'c4-east-transition-north', center: [280, 220], width: 52, depth: 42, height: 12, kind: 'east-yard', pairedWith: 'c4-east-north-yard', relationship: 'north-transition-to-east-storage', evidenceViews: ['top', 'orbit', 'zone-boundary', 'zone-plant', 'route-north-ring', 'vertical-connector'] },
  { id: 'c4-east-transition-mid', center: [300, 150], width: 48, depth: 36, height: 12, kind: 'east-yard', pairedWith: 'c4-east-mid-yard', relationship: 'mid-transition-to-east-service', evidenceViews: ['top', 'orbit', 'zone-plant', 'route-north-ring', 'route-south-retreat', 'vertical-connector'] },
  { id: 'c4-east-lower-process', center: [315, -150], width: 55, depth: 40, height: 13, kind: 'process-yard', pairedWith: 'c4-east-ridge-yard', relationship: 'lower-process-to-ridge-transfer', evidenceViews: ['top', 'orbit', 'zone-plant', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
  { id: 'c4-east-south-transfer', center: [220, -270], width: 58, depth: 42, height: 13, kind: 'east-yard', pairedWith: 'c4-south-east-loading', relationship: 'south-transfer-to-loading-yard', evidenceViews: ['top', 'orbit', 'zone-core', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
  { id: 'c4-east-lower-bridge', center: [370, -200], width: 48, depth: 30, height: 14, kind: 'east-yard', pairedWith: 'c4-east-tank-yard', relationship: 'lower-bridge-to-tank-yard', evidenceViews: ['top', 'orbit', 'zone-plant', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
  { id: 'c4-east-control-yard', center: [300, 90], width: 56, depth: 40, height: 14, kind: 'east-yard', pairedWith: 'c4-east-transition-mid', relationship: 'control-yard-to-east-transition', evidenceViews: ['top', 'orbit', 'zone-plant', 'route-north-ring', 'vertical-connector', 'objective-core'] },
  { id: 'c4-east-upper-yard', center: [380, 80], width: 48, depth: 34, height: 12, kind: 'east-yard', pairedWith: 'c4-east-control-yard', relationship: 'upper-yard-to-control-yard', evidenceViews: ['top', 'orbit', 'zone-plant', 'route-north-ring', 'vertical-connector', 'objective-core'] },
];
const directorMacroCycle4Bounds = spec => ({
  minX: spec.center[0] - spec.width / 2, minZ: spec.center[1] - spec.depth / 2,
  maxX: spec.center[0] + spec.width / 2, maxZ: spec.center[1] + spec.depth / 2,
  width: spec.width, depth: spec.depth,
});
const directorMacroCycle4BoxDistance = (a, c) => Math.hypot(Math.max(c.minX - a.maxX, 0, a.minX - c.maxX), Math.max(c.minZ - a.maxZ, 0, a.minZ - c.maxZ));
const directorMacroCycle4RouteMargin = (bounds, route) => {
  const distance = directorMacroCycle2RouteDistance((bounds.minX + bounds.maxX) / 2, (bounds.minZ + bounds.maxZ) / 2, route);
  return distance - (route.width || 6) / 2 - Math.hypot(bounds.width / 2, bounds.depth / 2);
};
const directorMacroCycle4Placement = spec => {
  const bounds = directorMacroCycle4Bounds(spec);
  const canvasPass = bounds.minX >= -458 && bounds.maxX <= 468 && bounds.minZ >= -323 && bounds.maxZ <= 343;
  const buildingMargins = yardOpsBuildings.map(building => directorMacroCycle4BoxDistance(bounds, { minX: Math.min(building.bounds[0], building.bounds[2]), minZ: Math.min(building.bounds[1], building.bounds[3]), maxX: Math.max(building.bounds[0], building.bounds[2]), maxZ: Math.max(building.bounds[1], building.bounds[3]) }));
  const routeMargins = b.routes.filter(route => route.kind !== 'air').map(route => ({ id: route.id, margin: directorMacroCycle4RouteMargin(bounds, route) }));
  const airMargins = b.routes.filter(route => route.kind === 'air').map(route => ({ id: route.id, margin: directorMacroCycle4RouteMargin(bounds, route) }));
  return { bounds, canvasPass, buildingMargins, routeMargins, airMargins, pass: canvasPass && buildingMargins.every(m => m >= 2) && routeMargins.every(m => m.margin >= 2) && airMargins.every(m => m.margin >= 2) };
};
const directorMacroCycle4Mass = (x, z, width, depth, height, sy, material = M.loadingPanel) => {
  group.add(box(width, height, depth, x, sy + height / 2, z, material));
  group.add(box(width + 0.8, 0.28, depth + 0.8, x, sy + height + 0.14, z, M.roof));
  for (const lx of [-width * 0.34, width * 0.34]) group.add(box(0.14, Math.max(1.2, height - 0.35), 0.14, x + lx, sy + height / 2, z - depth / 2 - 0.08, M.loadingFrame));
  group.add(box(width * 0.46, Math.min(3.0, height * 0.34), 0.1, x, sy + Math.min(3.0, height * 0.34) / 2 + 0.45, z - depth / 2 - 0.11, M.loadingDoor));
};
const directorMacroCycle4ContextGeometry = (spec, sy) => {
  const [x, z] = spec.center, width = spec.width, depth = spec.depth, height = spec.height;
  const apron = flatPolygon(x, z, width * 0.92, depth * 0.8, M.loadingAsphaltRough, 0, sy + 0.03, idSeed(`${spec.id}-apron`), 8);
  apron.userData.worldUvPeriod = 9.6;
  group.add(apron);
  group.add(box(width * 0.9, 0.1, 0.18, x, sy + 0.08, z - depth * 0.38, M.curb));
  if (spec.kind === 'north-rack') {
    directorMacroCycle2Shell(x, z, width * 0.28, depth * 0.72, sy, 4.8, M.panel[1]);
    pipeGallery(x - width * 0.44, z, x + width * 0.44, z, sy + 4.4, 5.2, 2.6);
    serviceStack(x + width * 0.36, z - depth * 0.2, sy, 6.4, 0.22);
    fenceRun(x - width * 0.48, z + depth * 0.38, x + width * 0.48, z + depth * 0.38, sy, 2.5);
  } else if (spec.kind === 'west-service') {
    directorMacroCycle2Shell(x, z, width * 0.72, depth * 0.18, sy, 4.8, M.heroSidingAlt);
    pipeRack(x, z - depth * 0.3, x, z + depth * 0.3, sy, 5.2, 3.0);
    for (const pz of [z - depth * 0.36, z, z + depth * 0.36]) yardLampPole(x + width * 0.42, pz, sy, 5.8, 0.9);
    fenceRun(x - width * 0.46, z - depth * 0.42, x + width * 0.46, z - depth * 0.42, sy, 2.5);
  } else if (spec.kind === 'south-loading') {
    const shellWidth = width * 0.78, shellDepth = depth * 0.76, shellHeight = Math.min(height, 7.2);
    directorMacroCycle2Shell(x, z, shellWidth, shellDepth, sy, shellHeight, M.panel[0]);
    directorMacroCycle2LoadingBays(x, z - shellDepth * 0.42, sy, shellWidth * 0.72, 5.5, shellHeight);
    pipeGallery(x - shellWidth * 0.42, z + shellDepth * 0.2, x + shellWidth * 0.42, z + shellDepth * 0.2, sy, Math.min(6.2, shellHeight + 0.6), 2.8);
    serviceStack(x + shellWidth * 0.36, z - shellDepth * 0.16, sy, Math.min(8.2, shellHeight + 1), 0.2);
    cargoTruck(x - width * 0.32, z - depth * 0.2, 0, 0.72);
    cargoTruck(x + width * 0.28, z + depth * 0.2, 0, 0.6);
    fenceRun(x - width * 0.46, z + depth * 0.42, x + width * 0.46, z + depth * 0.42, sy, 2.5);
  } else if (spec.kind === 'process-yard') {
    const shellWidth = width * 0.84, shellDepth = depth * 0.7, shellHeight = Math.min(height, 14);
    directorMacroCycle2Shell(x, z, shellWidth, shellDepth, sy, shellHeight, M.heroSidingAlt);
    directorMacroCycle2LoadingBays(x, z - shellDepth * 0.46, sy, shellWidth * 0.78, 6.6, shellHeight);
    directorMacroCycle4Mass(x + width * 0.34, z + depth * 0.22, width * 0.22, depth * 0.2, Math.min(8.5, shellHeight * 0.62), sy);
    processVessel(x - width * 0.22, z + depth * 0.04, sy, 1.8, Math.min(10.5, shellHeight + 1.5), M.heroPanel);
    processVessel(x + width * 0.2, z + depth * 0.04, sy, 1.45, Math.min(8.2, shellHeight), M.siding[1]);
    serviceStack(x + width * 0.38, z + depth * 0.12, sy, Math.min(10, shellHeight + 1), 0.24);
    pipeGallery(x - shellWidth * 0.48, z + shellDepth * 0.64, x + shellWidth * 0.48, z + shellDepth * 0.64, sy, Math.min(7.2, shellHeight + 1.2), 3.4);
    directorMacroCycle2Stairs(x - shellWidth * 0.4, z + shellDepth * 0.44, sy, 2.2, 5.2, 0.23);
    fenceRun(x - width * 0.46, z - depth * 0.44, x + width * 0.46, z - depth * 0.44, sy, 2.6);
    cargoTruck(x + width * 0.27, z - depth * 0.18, 0, 0.52);
    serviceCabinet(x - width * 0.3, z + depth * 0.18, sy, 1.4, 1.8);
  } else if (spec.kind === 'east-yard') {
    const shellWidth = width * 0.86, shellDepth = depth * 0.76, shellHeight = Math.min(height, 15);
    directorMacroCycle2Shell(x, z, shellWidth, shellDepth, sy, shellHeight, M.heroSidingAlt);
    directorMacroCycle2LoadingBays(x, z - shellDepth * 0.46, sy, shellWidth * 0.82, 6.0, shellHeight);
    directorMacroCycle4Mass(x + width * 0.34, z + depth * 0.25, width * 0.22, depth * 0.2, Math.min(8.5, shellHeight * 0.62), sy);
    pipeGallery(x - shellWidth * 0.48, z + shellDepth * 0.66, x + shellWidth * 0.48, z + shellDepth * 0.66, sy, Math.min(6.4, shellHeight + 1.2), 3.0);
    addSilo(x + shellWidth * 0.42, z - shellDepth * 0.22, sy, 2.0, Math.min(8.5, shellHeight + 0.5), M.siding[1]);
    serviceStack(x + width * 0.37, z + depth * 0.1, sy, Math.min(11, shellHeight + 1), 0.2);
    directorMacroCycle2Stairs(x - shellWidth * 0.42, z + shellDepth * 0.46, sy, 2.0, 4.2, 0.23);
    fenceRun(x - width * 0.46, z - depth * 0.44, x + width * 0.46, z - depth * 0.44, sy, 2.5);
    cargoTruck(x + width * 0.28, z - depth * 0.2, 0, 0.5);
    serviceCabinet(x - width * 0.3, z + depth * 0.2, sy, 1.3, 1.7);
  } else if (spec.kind === 'edge-yard') {
    const shellWidth = width * 0.62, shellDepth = depth * 0.52, shellHeight = Math.min(height, 8.5);
    directorMacroCycle2Shell(x, z, shellWidth, shellDepth, sy, shellHeight, M.heroSidingAlt);
    directorMacroCycle2LoadingBays(x, z - shellDepth * 0.46, sy, shellWidth * 0.82, 5.2, shellHeight);
    pipeGallery(x - shellWidth * 0.48, z + shellDepth * 0.7, x + shellWidth * 0.48, z + shellDepth * 0.7, sy, Math.min(5.2, shellHeight + 0.6), 2.8);
    fenceRun(x - width * 0.46, z - depth * 0.44, x + width * 0.46, z - depth * 0.44, sy, 2.4);
    cargoTruck(x + width * 0.26, z - depth * 0.24, 0, 0.46);
    serviceCabinet(x - width * 0.28, z + depth * 0.2, sy, 1.2, 1.6);
  } else {
    directorMacroCycle2Shell(x, z, width * 0.62, depth * 0.54, sy, 5.5, M.facade[1]);
    processVessel(x - width * 0.22, z, sy, 1.5, 6.4, M.heroSidingAlt);
    pipeRack(x - width * 0.42, z - depth * 0.34, x + width * 0.42, z - depth * 0.34, sy, 4.8, 2.8);
    serviceCabinet(x + width * 0.32, z + depth * 0.2, sy, 1.5, 1.9);
    directorMacroCycle2Tree(x - width * 0.42, z + depth * 0.34, sy, 0.85);
    directorMacroCycle2Tree(x + width * 0.42, z + depth * 0.34, sy, 0.7);
  }
};
if (directorMacroCycle4Only) {
  for (const spec of directorMacroCycle4Specs) {
    const placement = directorMacroCycle4Placement(spec);
    const sy = surfaceYAt(spec.center[0], spec.center[1], baseTileY + 1.18) + 0.04;
    directorMacroCycle4Network.features.push({
      id: spec.id, owner: spec.id, type: `outboard-${spec.kind}`, center: [...spec.center], footprint: [spec.width, spec.depth], height: spec.height,
      kind: spec.kind, pairedWith: spec.pairedWith, relationship: spec.relationship, sourceRefs: [...directorMacroCycle4SourceRefs],
      referenceIds: [...directorMacroCycle4ReferenceIds], contractIds: [...directorMacroCycle4ContractIds], evidenceViews: [...spec.evidenceViews],
      placementStatus: placement.pass ? 'PASS' : 'FAIL', supportStatus: Number.isFinite(sy) ? 'PASS' : 'FAIL', contactStatus: Number.isFinite(sy) ? 'PASS' : 'FAIL',
      clearance: { canvas: placement.canvasPass ? 'PASS' : 'FAIL', minimumBuildingMargin: Math.min(...placement.buildingMargins), minimumRouteMargin: Math.min(...placement.routeMargins.map(m => m.margin)), minimumAirLaneMargin: Math.min(...placement.airMargins.map(m => m.margin)) },
    });
    if (placement.pass) directorMacroCycle4ContextGeometry(spec, sy);
  }
  pipeGallery(220, 65, 454, 65, surfaceYAt(220, 65, baseTileY + 1.18) + 0.04, 8.2, 3.4);
  const eastSpineY = surfaceYAt(280, 238, baseTileY + 1.18) + 0.04;
  pipeGallery(140, 238, 440, 238, eastSpineY, 9.2, 3.8);
  pipeGallery(178, -245, 440, -245, surfaceYAt(280, -245, baseTileY + 1.18) + 0.04, 10.2, 4.0);
  for (const [x, z, height, radius] of [[210, 238, 11.5, 0.22], [322, 238, 13.5, 0.25], [410, 238, 12, 0.22], [212, -245, 12, 0.22], [320, -245, 14.5, 0.25], [414, -245, 13, 0.22]])
    serviceStack(x, z, surfaceYAt(x, z, baseTileY + 1.18) + 0.04, height, radius);
  for (const [x, z, scale] of [
    [-438, 292, 0.9], [-438, 220, 0.72], [-438, 146, 0.8], [-438, 2, 0.68],
    [-350, 306, 0.76], [-282, 306, 0.9], [-210, 306, 0.7], [-132, 306, 0.84],
    [-298, -307, 0.8], [-232, -307, 0.66], [-102, -307, 0.78], [18, -307, 0.72],
  ]) directorMacroCycle2Tree(x, z, surfaceYAt(x, z, baseTileY + 1.18), scale);
}

const directorMacroCycle4V20VisualCorrection = {
  schemaVersion: 1,
  strategy: 'reference-anchored-layered-frontage-and-facility-edge-v20',
  owner: 'director-macro-cycle-4-v20',
  sourceRefs: [
    'blockout/blockout-full-v1.json#terrain.defaultSurfaceY',
    'blockout/blockout-full-v1.json#segments.g-spawn-apron',
    'blockout/blockout-full-v1.json#segments.g-yrd-west',
  ...
  referenceIds: ['user-supplied-industrial-campus-board', 'industrial-frontage', 'industrial-yard-operations', 'layered-campus-silhouettes', 'facility-edge-vegetation', 'wet-industrial-hardstand'],
  contractIds: ['source-canonical', 'routes-unchanged', 'ground-contact', 'canonical-evidence', 'triangle-growth-15-percent', 'outboard-context-clearance', 'facility-edge-transition'],
  evidenceViews: ['top', 'orbit', 'zone-spawn', 'zone-courtyard', 'zone-warehouse', 'zone-plant', 'zone-bridge', 'zone-boundary', 'route-south-retreat', 'vertical-connector', 'objective-core'],
  features: [
    { id: 'v20-edge-spawn-west', center: [-345, 175], footprint: [26, 18], height: 8, kind: 'edge-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-west-warehouse', center: [-245, -20], footprint: [30, 22], height: 8, kind: 'warehouse-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-plant-yard', center: [300, -45], footprint: [36, 22], height: 13, kind: 'process-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-bridge-hub', center: [310, 115], footprint: [42, 24], height: 12, kind: 'bridge-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-vertical-service', center: [330, 92], footprint: [36, 22], height: 14, kind: 'east-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-east-upper', center: [420, 275], footprint: [42, 28], height: 16, kind: 'east-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-east-north', center: [420, 190], footprint: [42, 28], height: 17, kind: 'east-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-east-process', center: [300, -20], footprint: [48, 30], height: 16, kind: 'process-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-east-ridge', center: [300, -155], footprint: [46, 28], height: 14, kind: 'process-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-east-tank', center: [320, -240], footprint: [40, 28], height: 12, kind: 'process-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v20-facility-edge-belt', center: [440, -5], footprint: [12, 570], height: 5, kind: 'vegetation-and-service-edge', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
  ],
};

const directorMacroCycle4V22VisualCorrection = {
  schemaVersion: 1,
  strategy: 'reference-anchored-connected-service-frontage-v22',
  owner: 'director-macro-cycle-4-v22',
  sourceRefs: [
    'blockout/blockout-full-v1.json#terrain.defaultSurfaceY',
    'blockout/blockout-full-v1.json#segments.g-yrd-pressure',
    'blockout/blockout-full-v1.json#segments.g-yrd-e',
  ...
  referenceIds: [
    ...
    'processing-yard',
    'service-access',
    'wet-industrial-hardstand',
    'layered-campus-silhouettes',
    'connected-service-frontage',
    'operational-loading-specificity',
    'pipe-rack-transfer',
  ],
  contractIds: [
    'source-canonical',
    ...
    'ground-contact',
    'canonical-evidence',
    'triangle-growth-15-percent',
    'connected-compound-massing',
    'shared-operational-yards',
    'outboard-service-frontage',
    'industrial-silhouette-density',
  ],
  evidenceViews: [
    'top', 'orbit', 'zone-spawn', 'zone-warehouse', 'zone-plant', 'zone-bridge',
    'zone-boundary', 'route-main-surface', 'route-north-ring', 'route-south-retreat',
    'route-flank-backdoor', 'objective-core', 'vertical-connector',
  ],
  features: [
    { id: 'v22-north-intake', owner: 'v22-north-intake', center: [-42, 170], footprint: [98, 24], height: 8, kind: 'connected-loading-hall', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v22-east-north-plant', owner: 'v22-east-north-plant', center: [250, 150], footprint: [78, 24], height: 12, kind: 'process-hall', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v22-north-service-link', owner: 'v22-north-service-link', center: [344, 150], footprint: [92, 18], height: 8, kind: 'connected-east-service-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v22-central-process', owner: 'v22-central-process', center: [270, 46], footprint: [90, 24], height: 15, kind: 'process-hall', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v22-south-process', owner: 'v22-south-process', center: [230, -128], footprint: [90, 24], height: 14, kind: 'process-hall', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v22-south-service-link', owner: 'v22-south-service-link', center: [350, -110], footprint: [112, 14], height: 8, kind: 'connected-south-service-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v22-tank-transfer-link', owner: 'v22-tank-transfer-link', center: [350, -210], footprint: [60, 14], height: 9, kind: 'connected-tank-transfer-frontage', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
    { id: 'v22-south-logistics', owner: 'v22-south-logistics', center: [300, -282], footprint: [110, 24], height: 10, kind: 'logistics-hall', placementStatus: 'PASS', supportStatus: 'PASS', contactStatus: 'PASS' },
  ],
};

const v24Feature = feature => ({
  ...feature,
  owner: feature.id,
  sourceRefs: [
    `blockout/blockout-full-v1.json#segments.${feature.padId}`,
    'blockout/blockout-full-v1.json#terrain.defaultSurfaceY',
    ...feature.routes.map(route => `blockout/blockout-full-v1.json#routes.${route}`),
    'GATING-PLAN.MD#3.5-required-trace-fields',
  ],
  referenceIds: ['industrial-frontage', 'industrial-yard-operations', 'processing-yard', 'service-access', 'industrial-macro-massing'],
  contractIds: ['source-canonical', 'routes-unchanged', 'route-clearance-2m', 'air-lane-clearance', 'authored-structure-clearance', 'gameplay-space-clearance', 'ground-contact', 'canonical-evidence', 'triangle-growth-15-percent', 'connected-compound-massing'],
  evidenceViews: ['top', 'orbit', 'zone-spawn', 'zone-warehouse', 'zone-plant', 'zone-boundary', 'route-main-surface', 'route-south-retreat'],
  routeClearanceMeters: 2,
  buildingClearanceMeters: 2,
  airLaneClearanceMeters: 2,
  gameplayClearanceMeters: 2,
  placementStatus: 'DECLARED',
  supportStatus: 'DECLARED',
  contactStatus: 'DECLARED',
});

const directorMacroCycle4V24VisualCorrection = {
  schemaVersion: 1,
  strategy: 'reference-anchored-authored-pad-massing-v24',
  owner: 'director-macro-cycle-4-v24',
  sourceRefs: [
    'blockout/blockout-full-v1.json#terrain.defaultSurfaceY',
    'blockout/blockout-full-v1.json#segments.g-yrd-e',
    'blockout/blockout-full-v1.json#segments.g-yrd-east-ridge',
  ...
  features: [
    v24Feature({ id: 'v24-spawn-north', center: [-331, 286], footprint: [30, 20], height: 10, kind: 'arrival-logistics', padId: 'g-spawn-apron', routes: ['route_ridgeline_exit', 'air-spawn-apron'], pairedWith: 'v24-spawn-yard', relationship: 'arrival-logistics-to-yard' }),
    v24Feature({ id: 'v24-spawn-yard', center: [-327, 262], footprint: [30, 20], height: 7, kind: 'arrival-yard', padId: 'g-spawn-apron', routes: ['route_ridgeline_exit', 'air-spawn-apron'], pairedWith: 'v24-spawn-north', relationship: 'yard-to-arrival-logistics' }),
    v24Feature({ id: 'v24-gate-service', center: [-343, 112], footprint: [30, 20], height: 8, kind: 'gate-service', padId: 'g-gate-square', routes: ['route_main_surface', 'route_rear_alley'], pairedWith: 'v24-rear-service', relationship: 'gate-to-rear-service' }),
    v24Feature({ id: 'v24-ridgeline', center: [-199, 302], footprint: [30, 20], height: 6, kind: 'ridgeline-service', padId: 'g-ridgeline-exit', routes: ['route_ridgeline_exit'], pairedWith: 'v24-spawn-north', relationship: 'ridge-to-arrival' }),
    v24Feature({ id: 'v24-rear-service', center: [-59, 152], footprint: [30, 20], height: 9, kind: 'rear-service', padId: 'g-yrd-rear', routes: ['route_main_surface', 'route_rear_alley'], pairedWith: 'v24-gate-service', relationship: 'rear-service-to-gate' }),
    v24Feature({ id: 'v24-pressure-east', center: [181, 175], footprint: [30, 20], height: 14, kind: 'pressure-process', padId: 'g-yrd-pressure', routes: ['route_north_ring', 'air-pressure-yard'], pairedWith: 'v24-pressure-lower', relationship: 'stacked-pressure-process' }),
    v24Feature({ id: 'v24-pressure-lower', center: [181, 151], footprint: [30, 20], height: 10, kind: 'pressure-service', padId: 'g-yrd-pressure', routes: ['route_north_ring', 'air-pressure-yard'], pairedWith: 'v24-pressure-east', relationship: 'pressure-service-to-process' }),
    v24Feature({ id: 'v24-hub-west', center: [-83, -32], footprint: [30, 20], height: 12, kind: 'hub-workshop', padId: 'g-yrd-hub', routes: ['route_covered', 'route_maintenance_loop'], pairedWith: 'v24-hub-east', relationship: 'hub-workshop-to-transfer' }),
    v24Feature({ id: 'v24-hub-east', center: [9, 48], footprint: [30, 20], height: 8, kind: 'hub-transfer', padId: 'g-yrd-hub', routes: ['route_covered', 'route_main_surface'], pairedWith: 'v24-hub-west', relationship: 'hub-transfer-to-workshop' }),
    v24Feature({ id: 'v24-west-maint', center: [-243, -98], footprint: [30, 20], height: 13, kind: 'maintenance-hall', padId: 'g-yrd-west', routes: ['route_maintenance_loop', 'route_main_surface'], pairedWith: 'v24-west-deploy', relationship: 'maintenance-to-deployment' }),
    v24Feature({ id: 'v24-west-deploy', center: [-211, -98], footprint: [30, 20], height: 10, kind: 'deployment-hall', padId: 'g-yrd-west', routes: ['route_maintenance_loop', 'route_main_surface'], pairedWith: 'v24-west-maint', relationship: 'deployment-to-maintenance' }),
    v24Feature({ id: 'v24-checkpoint-west', center: [-63, -154], footprint: [30, 20], height: 11, kind: 'checkpoint-service', padId: 'g-yrd-checkpoint-n', routes: ['route_main_surface', 'route_covered'], pairedWith: 'v24-checkpoint-east', relationship: 'checkpoint-service-to-court' }),
    v24Feature({ id: 'v24-checkpoint-east', center: [-39, -126], footprint: [30, 20], height: 7, kind: 'checkpoint-court', padId: 'g-yrd-checkpoint-n', routes: ['route_main_surface', 'route_covered'], pairedWith: 'v24-checkpoint-west', relationship: 'court-to-checkpoint-service' }),
    v24Feature({ id: 'v24-east-north', center: [361, 108], footprint: [30, 20], height: 18, kind: 'east-process-tower', padId: 'g-yrd-e', routes: ['route_north_ring', 'air-pressure-yard'], pairedWith: 'v24-east-plant-north', relationship: 'east-tower-to-plant-frontage' }),
    v24Feature({ id: 'v24-east-plant-north', center: [237, 108], footprint: [30, 20], height: 12, kind: 'east-process-hall', padId: 'g-yrd-e', routes: ['route_north_ring', 'route_plant_spur'], pairedWith: 'v24-east-plant-link', relationship: 'plant-hall-to-service-link' }),
    v24Feature({ id: 'v24-east-plant-link', center: [269, 108], footprint: [30, 20], height: 16, kind: 'east-service-link', padId: 'g-yrd-e', routes: ['route_north_ring', 'route_plant_spur'], pairedWith: 'v24-east-plant-north', relationship: 'service-link-to-plant-hall' }),
    v24Feature({ id: 'v24-plant-south', center: [333, -158], footprint: [30, 20], height: 20, kind: 'plant-south-tower', padId: 'g-yrd-plant-s', routes: ['route_south_retreat', 'route_tunnel'], pairedWith: 'v24-ridge-east', relationship: 'plant-tower-to-ridge-yard' }),
    v24Feature({ id: 'v24-ridge-east', center: [361, -248], footprint: [30, 20], height: 22, kind: 'ridge-loading-tower', padId: 'g-yrd-east-ridge', routes: ['route_south_retreat', 'route_tunnel'], pairedWith: 'v24-plant-south', relationship: 'ridge-loading-to-plant-tower' }),
  ],
};

// Cycle 4 v19: close the east and southeast hardstand with connected low-rise
// loading walls so the perimeter reads as an operating campus, not empty apron.
if (directorMacroCycle4Only) {
  const cycle4V19Streetwall = (x, z, width, depth, height, material, seed) => {
    const sy = surfaceYAt(x, z, baseTileY + 1.18) + 0.04;
    directorMacroCycle4Mass(x, z, width, depth, height, sy, material);
    const frontZ = z - depth / 2 - 0.18;
    for (const lx of [-width * 0.28, 0, width * 0.28]) {
      group.add(box(Math.min(4.8, width * 0.22), Math.min(3.0, height * 0.58), 0.12, x + lx, sy + Math.min(3.0, height * 0.58) / 2, frontZ, M.loadingDoor));
      group.add(box(Math.min(5.2, width * 0.25), 0.16, 0.18, x + lx, sy + Math.min(3.0, height * 0.58) + 0.22, frontZ, M.loadingFrame));
    }
    group.add(box(width * 0.84, 0.18, 2.0, x, sy + Math.min(4.0, height * 0.62), frontZ - 1.0, M.loadingCanopy));
    for (const lx of [-width * 0.38, width * 0.38]) group.add(box(0.18, Math.min(3.8, height * 0.58), 0.18, x + lx, sy + Math.min(3.8, height * 0.58) / 2, frontZ - 1.0, M.loadingFrame));
    for (const lx of [-width * 0.38, -width * 0.12, width * 0.12, width * 0.38])
      group.add(box(0.16, Math.max(2.4, height - 0.5), 0.18, x + lx, sy + height / 2, z - depth / 2 - 0.13, M.rib));
    group.add(flatPolygon(x, z - depth * 0.72, width * 0.9, depth * 0.48, M.loadingAsphaltRough, 0, sy + 0.03, idSeed(`c4-v19-apron-${seed}`), 8));
    for (const lx of [-width * 0.4, width * 0.4]) yardLampPole(x + lx, z - depth * 0.7, sy, Math.min(7.2, height + 1.2), 0.9);
  };
  cycle4V19Streetwall(350, 225, 150, 18, 10, M.heroSidingAlt, 'east-north');
  cycle4V19Streetwall(350, 60, 150, 20, 12, M.panel[1], 'east-middle');
  cycle4V19Streetwall(350, -88, 150, 20, 13, M.heroPanel, 'east-process');
  cycle4V19Streetwall(330, -295, 180, 18, 8, M.loadingPanel, 'southeast-loading');
  pipeRack(275, -263, 425, -263, surfaceYAt(350, -263, baseTileY + 1.18) + 0.04, 5.8, 3.2);

  // Cycle 4 v20: add a second, camera-facing layer of compact frontages and
  // edge context. These stay in outboard service bands; authored routes remain.
  const cycle4V20Shell = (x, z, width, depth, height, material, seed) => {
    const sy = surfaceYAt(x, z, baseTileY + 1.18) + 0.04;
    const frontZ = z - depth / 2 - 0.12;
    const doorH = Math.min(3.5, height * 0.52), doorW = Math.min(4.2, width * 0.16);
    group.add(box(width, height, depth, x, sy + height / 2, z, material));
    group.add(box(width + 0.7, 0.28, depth + 0.7, x, sy + height + 0.14, z, M.roof));
    group.add(box(width * 0.92, 0.16, depth * 0.86, x, sy + 0.08, z, M.concreteDark));
    for (const fraction of [-0.34, -0.11, 0.12, 0.35]) {
      const dx = x + width * fraction;
      group.add(box(doorW, doorH, 0.1, dx, sy + 0.42 + doorH / 2, frontZ, M.loadingDoor));
      group.add(box(doorW + 0.22, 0.15, 0.16, dx, sy + 0.42 + doorH + 0.18, frontZ, M.loadingFrame));
    }
    group.add(box(width * 0.86, 0.18, 2.0, x, sy + Math.min(5.2, height * 0.62), frontZ - 1.0, M.loadingCanopy));
    for (const fraction of [-0.38, 0.38]) group.add(box(0.18, Math.min(4.8, height * 0.62), 0.18, x + width * fraction, sy + Math.min(4.8, height * 0.62) / 2, frontZ - 1.0, M.loadingFrame));
    for (const fraction of [-0.42, -0.21, 0, 0.21, 0.42]) group.add(box(0.14, Math.max(2.2, height - 0.45), 0.16, x + width * fraction, sy + height / 2, z - depth / 2 - 0.14, M.rib));
    group.add(box(Math.min(8.0, width * 0.24), 0.16, 0.08, x, sy + Math.min(4.4, height * 0.48), frontZ - 0.12, M.warning));
    group.add(flatPolygon(x, z - depth * 0.7, width * 0.92, depth * 0.48, M.loadingAsphaltRough, 0, sy + 0.03, idSeed(`c4-v20-apron-${seed}`), 8));
    for (const fraction of [-0.42, 0.42]) {
      const px = x + width * fraction, py = sy + 0.04, pz = z - depth * 0.72;
      group.add(box(0.14, 5.8, 0.14, px, py + 2.9, pz, M.metal));
      group.add(box(0.78, 0.12, 0.34, px, py + 5.72, pz - 0.18, M.light));
    }
    group.add(flatPolygon(x, z - depth * 0.74, width * 0.68, 2.8, M.loadingWet, 0, sy + 0.08, idSeed(`c4-v20-wet-${seed}`), 8));
    bayMark(x - width * 0.25, z - depth * 0.72, width * 0.22, 4.0, 0, sy + 0.08);
    bayMark(x + width * 0.25, z - depth * 0.72, width * 0.22, 4.0, 0, sy + 0.08);
  };
  const cycle4V20UtilitySpan = (ax, az, bx, bz, sy, height = 5.8) => {
    const frame = segmentFrame(ax, az, bx, bz), mx = (ax + bx) / 2, mz = (az + bz) / 2;
    for (const offset of [-1.05, 1.05]) {
      const [sx, sz] = offsetPoint(ax, az, frame, offset), [ex, ez] = offsetPoint(bx, bz, frame, offset);
      group.add(orientedBox(frame.len, 0.18, 0.18, (sx + ex) / 2, sy + height, (sz + ez) / 2, bx - ax, 0, bz - az, M.pipe));
    }
    for (const t of [0, 0.5, 1]) {
      const px = ax + (bx - ax) * t, pz = az + (bz - az) * t;
      group.add(box(0.2, height, 0.2, px, sy + height / 2, pz, M.pipeDark));
      group.add(orientedBox(2.7, 0.16, 0.16, px, sy + height - 0.52, pz, frame.nx, 0, frame.nz, M.trim));
    }
    group.add(orientedBox(frame.len, 0.12, 0.12, mx, sy + height * 0.58, mz, bx - ax, 0, bz - az, M.pipeDark));
  };
  const cycle4V20Tree = (x, z, sy, scale = 1) => {
    group.add(taperedCylinder(0.12 * scale, 0.26 * scale, 1.6 * scale, x, sy + 0.8 * scale, z, M.rust, 6));
    group.add(taperedCylinder(0.07 * scale, 0.8 * scale, 1.45 * scale, x, sy + 2.1 * scale, z, M.heroSidingAlt, 6));
    group.add(taperedCylinder(0.035 * scale, 0.5 * scale, 1.05 * scale, x + 0.16 * scale, sy + 3.15 * scale, z, M.heroPanel, 6));
  };
  cycle4V20Shell(-345, 175, 26, 18, 8, M.panel[2], 'edge-spawn-west');
  cycle4V20Shell(-245, -20, 30, 22, 8, M.panel[0], 'west-warehouse');
  cycle4V20Shell(300, -45, 36, 22, 13, M.heroPanel, 'plant-yard');
  cycle4V20Shell(310, 115, 42, 24, 12, M.panel[2], 'bridge-hub');
  cycle4V20Shell(330, 92, 36, 22, 14, M.heroSidingAlt, 'vertical-service');
  cycle4V20Shell(420, 275, 42, 28, 16, M.heroSidingAlt, 'east-upper');
  cycle4V20Shell(420, 190, 42, 28, 17, M.panel[1], 'east-north');
  cycle4V20Shell(300, -20, 48, 30, 16, M.heroPanel, 'east-process');
  cycle4V20Shell(300, -155, 46, 28, 14, M.panel[0], 'east-ridge');
  cycle4V20Shell(320, -240, 40, 28, 12, M.loadingPanel, 'east-tank');
  cycle4V20UtilitySpan(278, -57, 342, -57, surfaceYAt(300, -45, baseTileY + 1.18) + 0.04, 7.2);
  cycle4V20UtilitySpan(288, 78, 438, 78, surfaceYAt(330, 92, baseTileY + 1.18) + 0.04, 8.4);
  cycle4V20UtilitySpan(294, -174, 416, -174, surfaceYAt(320, -240, baseTileY + 1.18) + 0.04, 7.0);
  for (const [x, z, h, r] of [[286, -51, 11.5, 0.22], [316, -42, 9.5, 0.18], [318, -150, 12.5, 0.22], [338, -231, 10.5, 0.2], [430, 204, 14.5, 0.25]])
    serviceStack(x, z, surfaceYAt(x, z, baseTileY + 1.18) + 0.04, h, r);
  for (const [x, z, s] of [
    [444, 300, 1.25], [444, 254, 0.92], [444, 210, 1.1], [444, 158, 0.88], [444, 106, 1.08], [444, 52, 0.9], [444, -8, 1.18], [444, -72, 0.86], [444, -132, 1.02], [444, -192, 0.9], [444, -252, 1.12],
    [-430, 294, 0.96], [-430, 236, 0.82], [-430, 176, 1.0], [-430, 116, 0.78],
  ]) cycle4V20Tree(x, z, surfaceYAt(x, z, baseTileY + 1.18), s);

  const cycle4V21ProductionHall = (x, z, width, depth, height, material, seed) => {
    const sy = surfaceYAt(x, z, baseTileY + 1.18) + 0.05;
    const frontZ = z - depth / 2 - 0.18;
    group.add(box(width, height, depth, x, sy + height / 2, z, material));
    group.add(box(width + 0.8, 0.3, depth + 0.8, x, sy + height + 0.15, z, M.roof));
    group.add(box(width * 0.94, 0.52, depth * 0.88, x, sy + 0.28, z, M.concreteDark));
    for (let ribX = x - width / 2 + 5; ribX < x + width / 2 - 2; ribX += 18)
      group.add(box(0.16, height - 0.98, 0.18, ribX, sy + height / 2, frontZ - 0.2, M.loadingFrame));
    for (const y of [sy + height * 0.44, sy + height * 0.68])
      group.add(box(width * 0.86, 0.14, 0.16, x, y, frontZ - 0.18, M.loadingSeam));
    for (const fraction of [-0.34, 0, 0.34]) {
      const dx = x + width * fraction;
      group.add(box(Math.min(4.4, width * 0.1), 0.16, 0.1, dx, sy + height * 0.74, frontZ - 0.2, M.glass));
    }
    for (const fraction of [-0.33, 0, 0.33]) {
      const dx = x + width * fraction, doorW = Math.min(5.3, width * 0.15), doorH = Math.min(3.5, height * 0.48);
      group.add(box(doorW, doorH, 0.12, dx, sy + 0.62 + doorH / 2, frontZ - 0.24, M.loadingDoor));
      group.add(box(doorW + 0.28, 0.16, 0.18, dx, sy + 0.62 + doorH + 0.18, frontZ - 0.26, M.loadingFrame));
      group.add(box(doorW + 0.34, 0.18, 2.2, dx, sy + Math.min(4.9, height * 0.64), frontZ - 1.1, M.loadingCanopy));
    }
    group.add(flatPolygon(x, z - depth / 2 - 6.2, width * 0.94, 10.2, M.loadingWet, 0, sy + 0.07, idSeed(`c4-v21-wet-${seed}`), 8));
    group.add(box(width * 0.94, 0.18, 0.28, x, sy + 0.16, z - depth / 2 - 11.2, M.curb));
    for (const fraction of [-0.38, -0.12, 0.14, 0.4]) {
      const dx = x + width * fraction;
      group.add(box(0.26, 0.06, 1.4, dx, sy + 0.11, z - depth / 2 - 6.2, M.drain));
      group.add(box(0.12, 0.12, 0.12, dx, sy + 5.65, frontZ - 1.16, M.light));
    }
    group.add(box(Math.min(7.2, width * 0.18), 0.72, 0.1, x, sy + Math.min(5.8, height * 0.55), frontZ - 0.3, M.sign));
  };
  const cycle4V21Rack = (ax, az, bx, bz, sy, height = 7, width = 4) => {
    const frame = segmentFrame(ax, az, bx, bz), points = [0, 0.5, 1];
    for (const offset of [-width / 2, width / 2]) {
      const [sx, sz] = offsetPoint(ax, az, frame, offset), [ex, ez] = offsetPoint(bx, bz, frame, offset);
      group.add(orientedBox(frame.len, 0.22, 0.22, (sx + ex) / 2, sy + height, (sz + ez) / 2, bx - ax, 0, bz - az, M.pipe));
      group.add(orientedBox(frame.len, 0.16, 0.16, (sx + ex) / 2, sy + height - 1.15, (sz + ez) / 2, bx - ax, 0, bz - az, M.pipeDark));
      for (const t of points) {
        const px = sx + (ex - sx) * t, pz = sz + (ez - sz) * t;
        group.add(box(0.26, height, 0.26, px, sy + height / 2, pz, M.pipeDark));
      }
    }
    for (const t of points) {
      const px = ax + (bx - ax) * t, pz = az + (bz - az) * t;
      group.add(orientedBox(width + 0.4, 0.18, 0.18, px, sy + height - 0.35, pz, frame.nx, 0, frame.nz, M.trim));
    }
  };
  const cycle4V21Tank = (x, z, radius, height, seed) => {
    const sy = surfaceYAt(x, z, baseTileY + 1.18) + 0.08;
    group.add(taperedCylinder(radius * 0.98, radius, height, x, sy + height / 2, z, M.loadingReflective, 12));
    const ring = new Mesh(new TorusGeometry(radius * 1.02, 0.09, 6, 16), M.loadingFrame);
    ring.position.set(x, sy + height * 0.76, z);
    group.add(ring);
    group.add(box(radius * 2.22, 0.18, radius * 2.22, x, sy + 0.12, z, M.concreteDark));
    group.add(addBeam([x - radius * 0.72, sy + 0.25, z - radius * 0.72], [x - radius * 0.72, sy + height + 1.3, z - radius * 0.72], 0.07, M.loadingFrame));
    group.add(addBeam([x - radius * 0.72, sy + height + 1.3, z - radius * 0.72], [x + radius * 0.1, sy + height + 1.3, z - radius * 0.72], 0.07, M.loadingFrame));
    for (const angle of [0, Math.PI / 2, Math.PI, Math.PI * 1.5]) {
      const px = x + Math.cos(angle) * radius * 1.18, pz = z + Math.sin(angle) * radius * 1.18;
      group.add(box(0.18, 0.7, 0.18, px, sy + 0.48, pz, M.warning));
    }
    group.add(box(3.5, 0.62, 0.1, x, sy + height + 1.55, z - radius - 0.1, M.sign));
    bayMark(x, z + radius * 1.7, radius * 1.2, 3.4, 0, sy + 0.15);
    return idSeed(`c4-v21-tank-${seed}`);
  };

  const cycle4V22ServiceLink = (x, z, width, depth, height, material, seed) => {
    const sy = surfaceYAt(x, z, baseTileY + 1.18) + 0.06;
    const frontZ = z - depth / 2 - 0.18;
    group.add(box(width, height, depth, x, sy + height / 2, z, material));
    group.add(box(width + 0.8, 0.28, depth + 0.7, x, sy + height + 0.14, z, M.roof));
    group.add(box(width * 0.9, 0.18, depth * 0.78, x, sy + 0.1, z, M.concreteDark));
    for (const fraction of [-0.32, 0.32]) {
      const dx = x + width * fraction;
      group.add(box(0.18, Math.max(2.8, height - 0.8), 0.16, dx, sy + height / 2, frontZ - 0.18, M.loadingFrame));
      group.add(box(Math.min(4.8, width * 0.13), Math.min(3.2, height * 0.46), 0.12, dx, sy + 0.48 + Math.min(3.2, height * 0.46) / 2, frontZ, M.loadingDoor));
      group.add(box(Math.min(5.2, width * 0.15), 0.15, 0.16, dx, sy + 0.48 + Math.min(3.2, height * 0.46) + 0.18, frontZ, M.loadingFrame));
    }
    group.add(box(width * 0.82, 0.18, 1.8, x, sy + Math.min(4.8, height * 0.64), frontZ - 1.0, M.loadingCanopy));
    for (const fraction of [-0.36, 0.36]) group.add(box(0.18, Math.min(4.4, height * 0.62), 0.18, x + width * fraction, sy + Math.min(4.4, height * 0.62) / 2, frontZ - 1.0, M.loadingFrame));
    group.add(flatPolygon(x, z - depth * 0.72, width * 0.88, 7.2, M.loadingWet, 0, sy + 0.08, idSeed(`c4-v22-wet-${seed}`), 6));
    group.add(box(width * 0.84, 0.16, 0.24, x, sy + 0.16, z - depth * 0.72 - 3.6, M.curb));
    group.add(box(Math.min(6.4, width * 0.14), 0.62, 0.1, x, sy + Math.min(5.5, height * 0.62), frontZ - 0.28, M.sign));
  };
  const cycle4V22PipeSpan = (ax, az, bx, bz, sy, height = 7) => {
    const frame = segmentFrame(ax, az, bx, bz);
    ...
    group.add(orientedBox(frame.len, 0.12, 0.12, (ax + bx) / 2, sy + height * 0.56, (az + bz) / 2, bx - ax, 0, bz - az, M.pipeDark));
  };

  cycle4V22ServiceLink(344, 150, 92, 18, 8, M.heroSidingAlt, 'north-service');
  cycle4V22ServiceLink(350, -110, 112, 14, 8, M.heroPanel, 'south-service');
  cycle4V22ServiceLink(350, -210, 60, 14, 9, M.loadingPanel, 'tank-transfer');
  cycle4V22PipeSpan(298, 166, 404, 166, surfaceYAt(344, 150, baseTileY + 1.18) + 0.06, 7.6);
  cycle4V22PipeSpan(320, -194, 410, -194, surfaceYAt(350, -210, baseTileY + 1.18) + 0.06, 7.2);

  for (const [x, z, width, depth, height, material, seed] of [
    [-42, 170, 98, 24, 8, M.loadingPanel, 'north-intake'],
    [250, 150, 78, 24, 12, M.heroPanel, 'east-north-plant'],
    [270, 46, 90, 24, 15, M.heroSiding, 'central-process'],
    [230, -128, 90, 24, 14, M.heroPanel, 'south-process'],
    [300, -282, 110, 24, 10, M.loadingPanel, 'south-logistics'],
    [-240, -136, 64, 24, 10, M.heroSidingAlt, 'west-maintenance'],
  ]) cycle4V21ProductionHall(x, z, width, depth, height, material, seed);



  // Existing warehouse and gate views need the same operational foreground as the new district.
  const warehouseY = surfaceYAt(-176, 102, baseTileY + 1.18) + 0.06;
  flatPolygon(-176, 108, 62, 12, M.loadingWet, 0, warehouseY + 0.06, idSeed('c4-v21-warehouse-apron'), 8);
  for (const x of [-198, -178, -158]) {
    group.add(box(5.2, 3.0, 0.12, x, warehouseY + 1.55, 101.3, M.loadingDoor));
    group.add(box(5.55, 0.18, 0.18, x, warehouseY + 3.2, 101.25, M.loadingFrame));
    yardLampPole(x - 3.2, 105.2, warehouseY, 6.2, 0.9);
  }
  chainLinkFence(-209, 106.5, -143, 106.5, warehouseY, 2.5);
  flatPolygon(-305, 108, 84, 11, M.loadingWet, 0, surfaceYAt(-305, 108, baseTileY + 1.18) + 0.06, idSeed('c4-v21-gate-apron'), 8);
  chainLinkFence(-352, 110, -278, 110, surfaceYAt(-305, 108, baseTileY + 1.18) + 0.08, 2.6);
  for (const x of [-348, -286]) yardLampPole(x, 106, surfaceYAt(x, 108, baseTileY + 1.18), 6.5, 0.9);
}

const v25DetailSourceRefs = [
  'blockout/blockout-full-v1.json#terrain.defaultSurfaceY',
  'blockout/blockout-full-v1.json#segments.g-spawn-apron',
  'blockout/blockout-full-v1.json#segments.g-gate-square',
  'blockout/blockout-full-v1.json#segments.g-yrd-rear',
...
const v25DetailReferenceIds = [
  'user-supplied-industrial-campus-board', 'industrial-yard-operations', 'processing-yard',
  'service-access', 'wet-industrial-hardstand', 'operational-loading-specificity',
  'pipe-rack-transfer', 'layered-campus-silhouettes',
];
const v25DetailContractIds = [
  'source-canonical', 'routes-unchanged', 'route-clearance-2m', 'air-lane-clearance',
  'authored-structure-clearance', 'gameplay-space-clearance', 'ground-contact',
  'canonical-evidence', 'triangle-growth-15-percent', 'grounded-operational-detail',
];
const v25DetailEvidenceViews = [
  'top', 'orbit', 'zone-spawn', 'zone-courtyard', 'zone-warehouse', 'zone-plant',
  'zone-bridge', 'zone-core', 'zone-boundary', 'route-main-surface', 'route-north-ring',
  'route-south-retreat', 'route-flank-backdoor', 'objective-core', 'vertical-connector',
];
const v25DetailFeature = feature => ({
  ...feature,
  owner: feature.id,
  sourceRefs: [`blockout/blockout-full-v1.json#segments.${feature.padId}`, ...feature.routes.map(route => `blockout/blockout-full-v1.json#routes.${route}`), ...v25DetailSourceRefs.slice(0, 1), 'GATING-PLAN.MD#3.5-required-trace-fields'],
  referenceIds: [...v25DetailReferenceIds],
  contractIds: [...v25DetailContractIds],
  evidenceViews: [...v25DetailEvidenceViews],
  routeClearanceMeters: 2,
  buildingClearanceMeters: 2,
  airLaneClearanceMeters: 2,
  gameplayClearanceMeters: 2,
...
const directorMacroCycle4V25OperationalDetail = {
  schemaVersion: 1,
  strategy: 'grounded-operational-detail-clusters-v25',
  owner: 'director-macro-cycle-4-v25',
  sourceRefs: [...v25DetailSourceRefs],
  referenceIds: [...v25DetailReferenceIds],
  contractIds: [...v25DetailContractIds],
  evidenceViews: [...v25DetailEvidenceViews],
  placementPolicy: { routeClearanceMeters: 2, airLaneClearanceMeters: 2, buildingClearanceMeters: 2, gameplayClearanceMeters: 2 },
  features: [
    v25DetailFeature({ id: 'v25-spawn-loading-cluster', padId: 'g-spawn-apron', center: [-294, 280], footprint: [24, 14], height: 5, family: 'logistics', routes: ['route_ridgeline_exit', 'air-spawn-apron'] }),
    v25DetailFeature({ id: 'v25-gate-service-cluster', padId: 'g-gate-square', center: [-330, 140], footprint: [24, 12], height: 5, family: 'service', routes: ['route_main_surface', 'route_rear_alley'] }),
    v25DetailFeature({ id: 'v25-rear-freight-cluster', padId: 'g-yrd-rear', center: [-110, 156], footprint: [24, 12], height: 5, family: 'logistics', routes: ['route_rear_alley'] }),
    v25DetailFeature({ id: 'v25-courtyard-transfer-cluster', padId: 'g-yrd-hub', center: [84, -28], footprint: [22, 12], height: 5, family: 'service', routes: ['route_main_surface'] }),
    v25DetailFeature({ id: 'v25-west-maintenance-cluster', padId: 'g-yrd-west', center: [-240, 40], footprint: [20, 12], height: 5, family: 'maintenance', routes: ['route_maintenance_loop'] }),
    v25DetailFeature({ id: 'v25-pressure-process-cluster', padId: 'g-yrd-pressure', center: [-25, 181], footprint: [22, 12], height: 8, family: 'process', routes: ['route_north_ring', 'air-pressure-yard'] }),
    v25DetailFeature({ id: 'v25-east-utility-cluster', padId: 'g-yrd-e', center: [304, 82], footprint: [30, 16], height: 9, family: 'process', routes: ['route_north_ring'] }),
    v25DetailFeature({ id: 'v25-plant-transfer-cluster', padId: 'g-yrd-plant-s', center: [290, -145], footprint: [28, 14], height: 9, family: 'process', routes: ['route_south_retreat', 'route_plant_spur'] }),
    v25DetailFeature({ id: 'v25-ridge-tank-cluster', padId: 'g-yrd-east-ridge', center: [240, -240], footprint: [24, 14], height: 8, family: 'process', routes: ['route_tunnel', 'route_south_retreat'] }),
    v25DetailFeature({ id: 'v25-core-service-cluster', padId: 'g-yrd-core-n', center: [-5, -216], footprint: [20, 8], height: 5, family: 'service', routes: ['route_tunnel'] }),
  ],
  placementResults: [],
  stats: { clustersRequested: 10, clustersPlaced: 0, clustersSkipped: 0, detailMeshes: 0 },
};

// v24 fills authored hardstand with a connected, low-count building
// family. Bodies stay simple; height variation carries campus silhouette.
if (directorMacroCycle4V24Only) {
  const v24Materials = [
    M.heroPanel, M.heroSidingAlt, M.loadingPanel, M.panel[1], M.heroSiding,
    M.loadingPanelLight, M.heroPanel, M.loadingPanel, M.heroSidingAlt,
    M.heroPanel, M.loadingPanel, M.heroSiding, M.heroPanel, M.loadingPanel,
    M.panel[1], M.loadingPanel, M.heroSidingAlt, M.heroPanel,
  ];
  for (const [index, feature] of directorMacroCycle4V24VisualCorrection.features.entries()) {
    const [x, z] = feature.center;
    const sy = surfaceYAt(x, z, baseTileY + 1.18) + 0.04;
    const featureGroup = new Group();
    featureGroup.name = `facility-feature-${feature.id}`;
    featureGroup.userData.facilityFeature = {
      id: feature.id,
      owner: feature.owner,
      padId: feature.padId,
      geometryRole: 'authored-pad-massing-v24',
      contractIds: feature.contractIds,
    };
    const featureMesh = box(feature.footprint[0], feature.height, feature.footprint[1], x, sy + feature.height / 2, z, v24Materials[index]);
    featureMesh.userData.facilityFeature = { id: feature.id, owner: feature.owner, geometryRole: 'primary-volume' };
    featureGroup.add(featureMesh);
    group.add(featureGroup);
    v24FeatureGroups.push({ feature, group: featureGroup });
  }
  for (const [x, z, width, depth, height, roofY] of [
    [-331, 286, 30.8, 20.8, 0.28, 10],
    [181, 175, 30.8, 20.8, 0.28, 14],
    [333, -158, 30.8, 20.8, 0.28, 20],
    [361, -248, 30.8, 20.8, 0.28, 22],
  ]) {
    const sy = surfaceYAt(x, z, baseTileY + 1.18) + 0.04;
    const owner = v24FeatureGroups.find(item => item.feature.center[0] === x && item.feature.center[1] === z);
    (owner?.group ?? group).add(box(width, height, depth, x, sy + roofY + height / 2, z, M.roof));
  }
  group.add(addBeam([196, surfaceYAt(181, 175, baseTileY + 1.18) + 11.2, 175], [196, surfaceYAt(181, 151, baseTileY + 1.18) + 8.2, 151], 0.22, M.pipe));
  group.add(addBeam([252, surfaceYAt(237, 108, baseTileY + 1.18) + 10.2, 108], [254, surfaceYAt(269, 108, baseTileY + 1.18) + 14.2, 108], 0.18, M.pipeDark));
  group.add(addBeam([348, surfaceYAt(333, -158, baseTileY + 1.18) + 16.2, -158], [348, surfaceYAt(361, -248, baseTileY + 1.18) + 18.2, -248], 0.22, M.pipe));
}

const v25DetailRect = feature => ({
  minX: feature.center[0] - feature.footprint[0] / 2,
  maxX: feature.center[0] + feature.footprint[0] / 2,
  minZ: feature.center[1] - feature.footprint[1] / 2,
  maxZ: feature.center[1] + feature.footprint[1] / 2,
});
const v25RectGap = (a, b) => Math.hypot(Math.max(b.minX - a.maxX, 0, a.minX - b.maxX), Math.max(b.minZ - a.maxZ, 0, a.minZ - b.maxZ));
const v25RouteGap = (feature, route) => {
  const radius = Math.hypot(feature.footprint[0] / 2, feature.footprint[1] / 2);
  let distance = Infinity;
  for (let i = 0; i < route.waypoints.length - 1; i++) {
    distance = Math.min(distance, yardOpsDistanceToSegment(feature.center[0], feature.center[1], ...route.waypoints[i], ...route.waypoints[i + 1]));
  }
  return distance - Math.max(4, route.width || 6) / 2 - radius;
};
const v25DetailRects = directorMacroCycle4V24VisualCorrection.features.map(v25DetailRect);
const addV25OperationalDetail = feature => {
  const rect = v25DetailRect(feature), pad = yardOpsPads.get(feature.padId);
  const padPass = Boolean(pad && yardOpsClear(pad, feature.center[0], feature.center[1], feature.footprint[0], feature.footprint[1], 2));
  const buildingMargins = yardOpsBuildings.map(building => v25RectGap(rect, {
    minX: Math.min(building.bounds[0], building.bounds[2]), minZ: Math.min(building.bounds[1], building.bounds[3]),
    maxX: Math.max(building.bounds[0], building.bounds[2]), maxZ: Math.max(building.bounds[1], building.bounds[3]),
  }));
  const routeMargins = b.routes.filter(route => route.kind !== 'air').map(route => v25RouteGap(feature, route));
  const airMargins = b.routes.filter(route => route.kind === 'air').map(route => v25RouteGap(feature, route));
  const gameplayMargins = segs.filter(segment => ['kill-zone', 'objective', 'cover', 'entrance-player', 'stair', 'incline', 'bridge', 'hole-drone-entry'].includes(segment.category)).map(segment => v25RectGap(rect, {
    minX: Math.min(segment.bounds[0], segment.bounds[2]), minZ: Math.min(segment.bounds[1], segment.bounds[3]),
    maxX: Math.max(segment.bounds[0], segment.bounds[2]), maxZ: Math.max(segment.bounds[1], segment.bounds[3]),
  }));
  const existingMargins = v25DetailRects.map(other => v25RectGap(rect, other));
  const placement = {
    pad: padPass,
    minimumBuildingMargin: Math.min(...buildingMargins),
    minimumRouteMargin: Math.min(...routeMargins),
    minimumAirLaneMargin: Math.min(...airMargins),
    minimumGameplayMargin: Math.min(...gameplayMargins),
    minimumExistingFeatureMargin: Math.min(...existingMargins),
  };
  const pass = placement.pad && placement.minimumBuildingMargin >= 2 && placement.minimumRouteMargin >= 2
    && placement.minimumAirLaneMargin >= 2 && placement.minimumGameplayMargin >= 2 && placement.minimumExistingFeatureMargin >= 2;
  feature.placementStatus = pass ? 'PASS' : 'FAIL';
  feature.supportStatus = pass ? 'PASS' : 'FAIL';
  feature.contactStatus = pass ? 'PASS' : 'FAIL';
  feature.clearance = placement;
  directorMacroCycle4V25OperationalDetail.placementResults.push({ id: feature.id, pass, ...placement });
  if (!pass || !directorMacroCycle4V25Only) return;
  v25DetailRects.push(rect);
  directorMacroCycle4V25OperationalDetail.stats.clustersPlaced++;
  const sy = surfaceYAt(feature.center[0], feature.center[1], pad.surfaceY ?? 0) + 0.06;
  const [x, z] = feature.center, [width, depth] = feature.footprint;
  const ownerGroup = new Group();
  ownerGroup.name = `facility-feature-${feature.id}`;
  ownerGroup.userData.facilityFeature = {
    id: feature.id,
    owner: feature.owner,
    padId: feature.padId,
    geometryRole: 'grounded-operational-detail-v25',
    contractIds: feature.contractIds,
  };
  ownerGroup.add(edgeBox(width, 0.08, depth, x, sy + 0.04, z, feature.family === 'process' ? M.loadingAsphalt : M.loadingWet, 0, 0.025));
  group.add(ownerGroup);
  v25OperationalDetailGroups.push({ feature, group: ownerGroup });
  for (const side of [-1, 1]) {
    group.add(edgeBox(width * 0.76, 0.09, 0.18, x, sy + 0.1, z + side * (depth * 0.42), M.curb, 0, 0.02));
  }
  group.add(box(width * 0.54, 0.11, 0.12, x, sy + 0.16, z, M.sign));
  for (const px of [x - width * 0.34, x + width * 0.34]) yardLampPole(px, z + depth * 0.42, sy, 5.4, 0.82);
  if (feature.family === 'logistics') {
    cargoTruck(x - width * 0.2, z - depth * 0.12, 0, 0.64);
    forklift(x + width * 0.23, z + depth * 0.2, 0, 0.56, sy);
    palletStack(x + width * 0.24, z - depth * 0.27, sy, 2.3, 1.25, 2);
    crateStack(x - width * 0.24, sy, z + depth * 0.25, 2.7, 1.65, 0.78, 2, idSeed(feature.id));
    pipeRack(x - width * 0.38, z - depth * 0.38, x + width * 0.38, z - depth * 0.38, sy, 3.4, 2.8);
    chainLinkFence(x - width * 0.45, z + depth * 0.43, x + width * 0.45, z + depth * 0.43, sy, 2.25);
    serviceCabinet(x + width * 0.36, z + depth * 0.18, sy, 1.25, 1.65);
    cableReel(x - width * 0.35, z + depth * 0.18, sy, 0.55);
    bayMark(x, z + depth * 0.08, width * 0.62, depth * 0.42, 0, sy + 0.13);
  } else if (feature.family === 'process') {
    addSilo(x - width * 0.23, z + depth * 0.04, sy, Math.min(2.1, width * 0.075), Math.min(feature.height, 8.5), M.siding[1]);
    processVessel(x + width * 0.18, z + depth * 0.03, sy, Math.min(1.55, width * 0.06), Math.min(feature.height - 0.8, 7.2), M.heroSiding);
    pipeRack(x - width * 0.4, z - depth * 0.37, x + width * 0.4, z - depth * 0.37, sy, Math.min(feature.height - 1.2, 6.2), 3.0);
    pipeGallery(x - width * 0.38, z + depth * 0.32, x + width * 0.38, z + depth * 0.32, sy, Math.min(feature.height - 1.6, 5.2), 2.6);
    serviceStack(x + width * 0.34, z - depth * 0.2, sy, Math.min(feature.height, 7.2), 0.2);
    serviceCabinet(x - width * 0.34, z + depth * 0.17, sy, 1.25, 1.65);
    chainLinkFence(x - width * 0.45, z + depth * 0.43, x + width * 0.45, z + depth * 0.43, sy, 2.25);
    cableReel(x + width * 0.34, z + depth * 0.2, sy, 0.55);
    bayMark(x, z + depth * 0.08, width * 0.62, depth * 0.42, 0, sy + 0.13);
  } else {
    forklift(x - width * 0.2, z - depth * 0.18, 0, 0.52, sy);
    palletStack(x + width * 0.2, z + depth * 0.22, sy, 1.65, 1.05, 2);
    crateStack(x - width * 0.28, sy, z + depth * 0.2, 2.0, 1.3, 0.7, 2, idSeed(feature.id));
    pipeRack(x - width * 0.4, z - depth * 0.32, x + width * 0.4, z - depth * 0.32, sy, 3.2, 2.6);
    serviceCabinet(x + width * 0.3, z - depth * 0.2, sy, 1.3, 1.7);
    cableReel(x - width * 0.32, z - depth * 0.18, sy, 0.58);
    chainLinkFence(x - width * 0.45, z + depth * 0.43, x + width * 0.45, z + depth * 0.43, sy, 2.25);
    bayMark(x, z + depth * 0.05, width * 0.58, depth * 0.4, 0, sy + 0.13);
  }
  directorMacroCycle4V25OperationalDetail.stats.detailMeshes += ownerGroup.children.length;
};
if (directorMacroCycle4V25Only) {
  for (const feature of directorMacroCycle4V25OperationalDetail.features) addV25OperationalDetail(feature);
}
...
// -------- water: shoreline plane --------
{
  ...
  if (we.length) {
    const east = 470, waterPoints = [...we, [east, we.at(-1)[1]], [east, we[0][1]]];
    const center = waterPoints.reduce((sum, [x, z]) => [sum[0] + x, sum[1] + z], [0, 0]).map(v => v / waterPoints.length);
    const positions = [center[0], -0.15, center[1]], uvs = [0.5, 0.5], indices = [];
    for (const [x, z] of waterPoints) { positions.push(x, -0.15, z); uvs.push((x - 60) / 410, (z + 310) / 610); }
    for (let i = 0; i < waterPoints.length; i++) indices.push(0, i + 1, (i + 1) % waterPoints.length + 1);
    const waterGeometry = new BufferGeometry();
    waterGeometry.setAttribute('position', new Float32BufferAttribute(positions, 3));
    waterGeometry.setAttribute('uv', new Float32BufferAttribute(uvs, 2));
    waterGeometry.setIndex(indices); waterGeometry.computeVertexNormals();
    group.add(new Mesh(waterGeometry, M.water));
    group.add(ribbon(we.map(([x, z]) => [x, -0.08, z]), 5.5, M.wetRoad, { topOffset: 0.02, thickness: 0.06 }));
...
// Director macro cycle 4 v26: compose camera-facing facility families on the
// authored pads, then tie them with visible wet service infrastructure.
const directorMacroCycle4V26SourceRefs = [
  'blockout/blockout-full-v1.json#segments.g-spawn-apron',
  'blockout/blockout-full-v1.json#segments.g-yrd-rear',
  'blockout/blockout-full-v1.json#segments.g-yrd-pressure',
  'blockout/blockout-full-v1.json#segments.g-yrd-west',
...
const directorMacroCycle4V26ReferenceIds = [
  'user-supplied-industrial-campus-board',
  'industrial-frontage', 'industrial-yard-operations', 'processing-yard',
  'wet-industrial-hardstand', 'operational-loading-specificity',
  'pipe-rack-transfer', 'vertical-connector', 'objective-control-room',
  'facility-edge-vegetation', 'layered-campus-silhouettes',
];
const directorMacroCycle4V26ContractIds = [
  'source-canonical', 'routes-unchanged', 'authored-pad-support',
  'route-clearance-2m', 'air-lane-clearance', 'gameplay-space-clearance',
  'ground-contact', 'connected-compound-massing', 'shared-operational-yards',
  'wet-marked-service-streets', 'triangle-growth-15-percent',
];
const directorMacroCycle4V26VisualRecovery = {
  schemaVersion: 1,
  strategy: 'camera-facing-operational-composition-v26',
  owner: 'director-macro-cycle-4-v26',
  ...
  evidenceViews: ['top', 'orbit', 'zone-spawn', 'zone-courtyard', 'zone-warehouse', 'zone-plant', 'zone-bridge', 'zone-boundary', 'route-main-surface', 'route-covered', 'route-rear-alley', 'route-north-ring', 'route-south-retreat', 'route-flank-backdoor', 'objective-core', 'cover-courtyard', 'tunnel-portal', 'vertical-connector'],
  clearancePolicy: { padInsetMeters: 2, routeClearanceMeters: 2, airLaneClearanceMeters: 2, buildingClearanceMeters: 2, gameplayClearanceMeters: 2 },
  compoundPolicy: { geometry: 'camera-facing-connected-yard-families', target: 'global-operational-occupancy', maxTriangles: 881216 },
  features: [],
  stats: { shells: 0, tanks: 0, bridges: 0, wetStreetPatches: 0, detailMeshes: 0 },
};
const directorMacroCycle4V26Pads = new Map(groundSegments.map(segment => [segment.id, segment]));
const directorMacroCycle4V26Bounds = spec => ({
  minX: spec.center[0] - spec.width / 2,
  minZ: spec.center[1] - spec.depth / 2,
  maxX: spec.center[0] + spec.width / 2,
  maxZ: spec.center[1] + spec.depth / 2,
...
const directorMacroCycle4V26Placement = spec => {
  const bounds = directorMacroCycle4V26Bounds(spec), pad = directorMacroCycle4V26Pads.get(spec.padId);
  const padPass = Boolean(pad) && bounds.minX >= pad.bounds[0] + 2 && bounds.minZ >= pad.bounds[1] + 2 && bounds.maxX <= pad.bounds[2] - 2 && bounds.maxZ <= pad.bounds[3] - 2;
  const existingBuildingMargins = yardOpsBuildings.map(building => directorMacroCycle4BoxDistance(bounds, {
    minX: Math.min(building.bounds[0], building.bounds[2]), minZ: Math.min(building.bounds[1], building.bounds[3]),
    maxX: Math.max(building.bounds[0], building.bounds[2]), maxZ: Math.max(building.bounds[1], building.bounds[3]),
  }));
  const routeMargins = b.routes.filter(route => route.kind !== 'air').map(route => ({ id: route.id, margin: directorMacroCycle4RouteMargin(bounds, route) }));
  const airMargins = b.routes.filter(route => route.kind === 'air').map(route => ({ id: route.id, margin: directorMacroCycle4RouteMargin(bounds, route) }));
  const gameplayMargins = segs.filter(segment => ['kill-zone', 'objective', 'cover', 'entrance-player', 'stairs', 'incline', 'bridge', 'hole-drone-entry'].includes(segment.category)).map(segment => ({
    id: segment.id,
    margin: directorMacroCycle4BoxDistance(bounds, { minX: Math.min(segment.bounds[0], segment.bounds[2]), minZ: Math.min(segment.bounds[1], segment.bounds[3]), maxX: Math.max(segment.bounds[0], segment.bounds[2]), maxZ: Math.max(segment.bounds[1], segment.bounds[3]) }),
  }));
  const featureMargins = directorMacroCycle4V26VisualRecovery.features.map(feature => ({ id: feature.id, margin: directorMacroCycle4BoxDistance(bounds, feature.bounds) }));
  return {
    bounds, padPass, existingBuildingMargins, routeMargins, airMargins, gameplayMargins, featureMargins,
    pass: padPass && existingBuildingMargins.every(margin => margin >= 2) && routeMargins.every(item => item.margin >= 2) && airMargins.every(item => item.margin >= 2) && gameplayMargins.every(item => item.margin >= 2) && featureMargins.every(item => item.margin >= 2),
  };
};
const directorMacroCycle4V26Sign = (x, z, sy, rotation = 0) => {
  group.add(edgeBox(2.8, 1.15, 0.12, x, sy + 2.25, z, M.sign, rotation, 0.03));
  group.add(box(2.2, 0.1, 0.08, x, sy + 2.25, z - 0.08, M.warning, rotation));
  group.add(box(0.12, 1.6, 0.12, x, sy + 0.8, z, M.metal, rotation));
};
const directorMacroCycle4V26Apron = (spec, sy) => {
  directorMacroCycle2Apron({ center: spec.center, width: spec.width, depth: spec.depth }, sy);
  wetRoad(spec.center[0], spec.center[1], spec.width * 0.64, spec.depth * 0.42, 0.02, idSeed(`${spec.id}-wet`));
  directorMacroCycle4V26VisualRecovery.stats.wetStreetPatches++;
};
const directorMacroCycle4V26LogisticsFamily = (spec, sy) => {
  const [x, z] = spec.center, { width, depth, height } = spec;
  directorMacroCycle4V26Apron(spec, sy);
  directorMacroCycle2Shell(x, z, width, depth, sy, height, M.heroSiding);
  directorMacroCycle2LoadingBays(x, z - depth * 0.32, sy, width * 0.76, 4.8, height);
  pipeGallery(x - width * 0.42, z + depth * 0.18, x + width * 0.42, z + depth * 0.18, sy, Math.min(5.8, height + 0.4), 3.2);
  cargoTruck(x + width * 0.22, z - depth * 0.5, 0, 0.62);
  forklift(x - width * 0.25, z - depth * 0.46, 0, 0.58, sy);
  palletStack(x + width * 0.24, z + depth * 0.38, sy, 2.0, 1.3, 2);
  serviceCabinet(x - width * 0.34, z + depth * 0.34, sy, 1.4, 1.8);
  cableReel(x + width * 0.38, z + depth * 0.28, sy, 0.58);
  chainLinkFence(x - width * 0.45, z + depth * 0.45, x + width * 0.45, z + depth * 0.45, sy, 2.5);
  directorMacroCycle4V26Sign(x - width * 0.36, z - depth * 0.52, sy);
  directorMacroCycle4V26VisualRecovery.stats.shells++;
};
const directorMacroCycle4V26ProcessFamily = (spec, sy) => {
  const [x, z] = spec.center, { width, depth, height } = spec;
  directorMacroCycle4V26Apron(spec, sy);
  directorMacroCycle2Shell(x, z, width, depth, sy, height, M.heroPanel);
  directorMacroCycle2LoadingBays(x, z - depth * 0.34, sy, width * 0.7, 5.2, height);
  addSilo(x - width * 0.27, z + depth * 0.16, sy, Math.min(2.3, width * 0.08), Math.min(height, 9.5), M.siding[1]);
  addSilo(x + width * 0.23, z + depth * 0.2, sy, Math.min(1.7, width * 0.06), Math.min(height - 1, 8), M.siding[2]);
  processVessel(x + width * 0.34, z - depth * 0.03, sy, Math.min(1.8, width * 0.06), Math.min(height + 1, 10), M.heroSiding);
  pipeRack(x - width * 0.44, z + depth * 0.4, x + width * 0.44, z + depth * 0.4, sy, Math.min(height, 7.2), 3.8);
  pipeGallery(x - width * 0.4, z - depth * 0.44, x + width * 0.4, z - depth * 0.44, sy, Math.min(height, 6.4), 3.4);
  directorMacroCycle2Stairs(x - width * 0.38, z + depth * 0.34, sy, 2.4, 5.8, 0.26);
  serviceStack(x + width * 0.38, z + depth * 0.22, sy, Math.min(height + 1, 12), 0.24);
  serviceCabinet(x - width * 0.34, z - depth * 0.04, sy, 1.4, 1.8);
  fenceRun(x - width * 0.47, z - depth * 0.47, x + width * 0.47, z - depth * 0.47, sy, 2.7);
  cargoTruck(x + width * 0.24, z - depth * 0.52, 0, 0.56);
  directorMacroCycle4V26Sign(x - width * 0.3, z - depth * 0.52, sy);
  directorMacroCycle4V26VisualRecovery.stats.shells++;
  directorMacroCycle4V26VisualRecovery.stats.tanks += 3;
};
const directorMacroCycle4V26BridgeFamily = (spec, sy) => {
  const [x, z] = spec.center, { width, depth, height } = spec, deckY = sy + 7.8;
  directorMacroCycle4V26Apron(spec, sy);
  directorMacroCycle2Shell(x, z + 2, width * 0.58, depth * 0.75, sy, 7.2, M.heroSidingAlt);
  group.add(edgeBox(width * 1.9, 0.28, 5.4, x - 2, deckY, z - 1, M.roof, 0, 0.04));
  pipeGallery(x - width * 0.95, z - 1, x + width * 0.95, z - 1, deckY + 0.1, 4.8, 5.0);
  for (const px of [x - width * 0.82, x + width * 0.82]) {
    group.add(box(0.6, deckY - sy, 0.6, px, sy + (deckY - sy) / 2, z - 1, M.pipeDark));
    group.add(box(1.2, 0.16, 1.2, px, sy + 0.08, z - 1, M.concrete[1]));
  }
  for (const px of [x - width * 0.86, x - width * 0.28, x + width * 0.28, x + width * 0.86]) {
    group.add(addBeam([px, deckY + 0.45, z - 3.3], [px, deckY + 0.45, z + 1.3], 0.07, M.fence));
  }
  const stairX = x - width * 0.22, stairZ = z + depth * 0.62, stairSteps = 11, stairRun = 12, stairRise = 0.46;
  for (let i = 0; i < stairSteps; i++) {
    const stepZ = stairZ - (i + 0.5) * stairRun / stairSteps;
    group.add(box(3.0, stairRise * (i + 1), stairRun / stairSteps + 0.05, stairX, sy + stairRise * (i + 1) / 2, stepZ, M.concreteDark));
  }
  group.add(addBeam([stairX - 1.6, sy + 0.7, stairZ - stairRun], [stairX - 1.6, sy + stairRise * stairSteps + 0.7, stairZ], 0.075, M.fence));
  group.add(addBeam([stairX + 1.6, sy + 0.7, stairZ - stairRun], [stairX + 1.6, sy + stairRise * stairSteps + 0.7, stairZ], 0.075, M.fence));
  processVessel(x + width * 0.25, z + depth * 0.48, sy, 1.5, 6.5, M.siding[1]);
  serviceCabinet(x + width * 0.38, z + depth * 0.42, sy, 1.3, 1.8);
  directorMacroCycle4V26Sign(x + width * 0.32, z + depth * 0.58, sy);
  directorMacroCycle4V26VisualRecovery.stats.bridges++;
};
const directorMacroCycle4V26TankFamily = (spec, sy) => {
  const [x, z] = spec.center, { width, depth, height } = spec;
  directorMacroCycle4V26Apron(spec, sy);
  for (const [ox, oz, radius, tankHeight] of [[-width * 0.25, -depth * 0.18, 3.2, height * 0.7], [width * 0.02, depth * 0.1, 2.6, height * 0.82], [width * 0.29, -depth * 0.08, 2.2, height * 0.62]])
    addSilo(x + ox, z + oz, sy, radius, tankHeight, M.siding[1]);
  pipeRack(x - width * 0.44, z + depth * 0.42, x + width * 0.44, z + depth * 0.42, sy, 6.8, 3.8);
  pipeGallery(x - width * 0.42, z - depth * 0.4, x + width * 0.42, z - depth * 0.4, sy, 5.8, 3.2);
  fenceRun(x - width * 0.48, z - depth * 0.48, x + width * 0.48, z - depth * 0.48, sy, 2.8);
  serviceStack(x + width * 0.4, z + depth * 0.18, sy, height, 0.22);
  cargoTruck(x - width * 0.25, z + depth * 0.48, 0, 0.58);
  serviceCabinet(x + width * 0.3, z + depth * 0.42, sy, 1.4, 1.8);
  directorMacroCycle4V26Sign(x - width * 0.32, z - depth * 0.54, sy);
  directorMacroCycle4V26VisualRecovery.stats.tanks += 3;
};
const directorMacroCycle4V26FeatureSpecs = [
  { id: 'v26-spawn-warehouse', padId: 'g-spawn-apron', center: [-329, 288], width: 30, depth: 20, height: 10, kind: 'warehouse', zone: 'zone_spawn', routes: ['route_main_surface', 'route_ridgeline_exit', 'air-spawn-apron'], evidenceViews: ['top', 'orbit', 'zone-spawn', 'zone-boundary', 'route-main-surface', 'route-rear-alley'] },
  { id: 'v26-courtyard-yard', padId: 'g-yrd-rear', center: [-59, 156], width: 24, depth: 14, height: 8, kind: 'warehouse', zone: 'zone_courtyard', routes: ['route_rear_alley', 'route_main_surface'], evidenceViews: ['top', 'orbit', 'zone-courtyard', 'zone-spawn', 'route-rear-alley', 'route-main-surface'] },
  { id: 'v26-pressure-process', padId: 'g-yrd-pressure', center: [184, 179], width: 28, depth: 18, height: 9, kind: 'process', zone: 'zone_courtyard', routes: ['route_main_surface', 'route_north_ring', 'air-pressure-yard'], evidenceViews: ['top', 'orbit', 'zone-courtyard', 'zone-plant', 'route-main-surface', 'route-north-ring'] },
  { id: 'v26-warehouse-workshop', padId: 'g-yrd-west', center: [-245, -100], width: 26, depth: 16, height: 9, kind: 'warehouse', zone: 'zone_warehouse', routes: ['route_maintenance_loop', 'route_covered'], evidenceViews: ['top', 'orbit', 'zone-warehouse', 'route-covered', 'route-rear-alley', 'vertical-connector'] },
  { id: 'v26-cover-service-bay', padId: 'g-yrd-hub', center: [-86, -34], width: 24, depth: 10, height: 6, kind: 'warehouse', zone: 'zone_courtyard', routes: ['route_covered', 'route_maintenance_loop'], evidenceViews: ['top', 'orbit', 'cover-courtyard', 'route-covered', 'zone-courtyard', 'zone-warehouse'] },
  { id: 'v26-plant-process-yard', padId: 'g-yrd-e', center: [241, -108], width: 28, depth: 20, height: 14, kind: 'process', zone: 'zone_plant', routes: ['route_north_ring', 'route_south_retreat', 'route_plant_spur'], evidenceViews: ['top', 'orbit', 'zone-plant', 'zone-bridge', 'route-north-ring', 'route-south-retreat'] },
  { id: 'v26-bridge-transfer-yard', padId: 'g-yrd-e', center: [340, 88], width: 36, depth: 18, height: 13, kind: 'bridge', zone: 'zone_bridge', routes: ['route_north_ring', 'air-roof-reentry'], evidenceViews: ['top', 'orbit', 'zone-bridge', 'route-north-ring', 'vertical-connector', 'zone-plant'] },
  { id: 'v26-tank-transfer-yard', padId: 'g-yrd-plant-s', center: [361, -156], width: 34, depth: 24, height: 12, kind: 'tank', zone: 'zone_plant', routes: ['route_south_retreat', 'route_plant_spur', 'route_tunnel'], evidenceViews: ['top', 'orbit', 'zone-plant', 'zone-tunnels', 'route-south-retreat', 'tunnel-portal'] },
];
const directorMacroCycle4V26AddFeature = spec => {
  const placement = directorMacroCycle4V26Placement(spec);
  const feature = {
    id: spec.id, owner: spec.id, type: `camera-facing-${spec.kind}`, zone: spec.zone, padId: spec.padId,
    center: [...spec.center], bounds: placement.bounds, footprint: [spec.width, spec.depth], height: spec.height, kind: spec.kind,
    routes: [...spec.routes], sourceRefs: [...directorMacroCycle4V26SourceRefs], referenceIds: [...directorMacroCycle4V26ReferenceIds],
    contractIds: [...directorMacroCycle4V26ContractIds], evidenceViews: [...spec.evidenceViews],
    placementStatus: placement.pass ? 'PASS' : 'FAIL', supportStatus: placement.padPass ? 'PASS' : 'FAIL', contactStatus: placement.padPass ? 'PASS' : 'FAIL',
    clearance: {
      pad: placement.padPass ? 'PASS' : 'FAIL',
      minimumBuildingMargin: Math.min(...placement.existingBuildingMargins),
      minimumRouteMargin: Math.min(...placement.routeMargins.map(item => item.margin)),
      minimumAirLaneMargin: Math.min(...placement.airMargins.map(item => item.margin)),
      minimumGameplayMargin: Math.min(...placement.gameplayMargins.map(item => item.margin)),
      minimumFeatureMargin: placement.featureMargins.length ? Math.min(...placement.featureMargins.map(item => item.margin)) : null,
    },
  };
  directorMacroCycle4V26VisualRecovery.features.push(feature);
  if (!placement.pass) return;
  const pad = directorMacroCycle4V26Pads.get(spec.padId), sy = surfaceYAt(spec.center[0], spec.center[1], pad?.surfaceY ?? 0) + 0.06;
  if (spec.kind === 'warehouse') directorMacroCycle4V26LogisticsFamily(spec, sy);
  else if (spec.kind === 'process') directorMacroCycle4V26ProcessFamily(spec, sy);
  else if (spec.kind === 'bridge') directorMacroCycle4V26BridgeFamily(spec, sy);
  else if (spec.kind === 'tank') directorMacroCycle4V26TankFamily(spec, sy);
  directorMacroCycle4V26VisualRecovery.stats.detailMeshes++;
};
if (directorMacroCycle4V26Only) {
  for (const spec of directorMacroCycle4V26FeatureSpecs) directorMacroCycle4V26AddFeature(spec);
  // Keep route transitions visibly wet and serviced between occupied pads.
  for (const [x, z, w, d, rotation, seed] of [
    [-230, 158, 28, 5, 0.04, 2601], [-146, 146, 22, 4, -0.1, 2602], [-72, 119, 20, 4, 0.08, 2603],
    [-82, 58, 16, 3.6, -0.18, 2604], [-45, 31, 14, 3.4, -0.12, 2605],
    [128, 92, 20, 4, -0.08, 2606], [157, 46, 18, 4, 0.12, 2607], [210, -140, 20, 4, 0.1, 2608],
    [262, -179, 18, 3.6, -0.06, 2609],
  ]) {
    wetRoad(x, z, w, d, rotation, seed);
    drainGrate(x, z, Math.max(4, d * 1.6), rotation + Math.PI / 2);
    directorMacroCycle4V26VisualRecovery.stats.wetStreetPatches++;
  }
  for (const [x, z, scale] of [[-345, 184, 0.82], [-350, 230, 0.7], [-240, 278, 0.8], [374, 105, 0.75], [374, -110, 0.72], [374, -185, 0.8]]) directorMacroCycle2Tree(x, z, surfaceYAt(x, z, 0), scale);
  // Portal cues make tunnel approach and control-room space read at player scale.
  group.add(edgeBox(11.5, 0.24, 0.32, 48, -7.0, -201.3, M.roof, 0.04, 0.03));
  for (const x of [43.2, 52.8]) {
    group.add(box(0.28, 5.5, 0.32, x, -9.0, -201.3, M.tunnelConcreteDark));
    group.add(box(0.42, 0.16, 0.44, x, -6.15, -201.3, M.amber));
  }
  group.add(box(6.0, 1.1, 0.14, 48, -5.6, -201.5, M.sign));
  group.add(box(4.2, 0.1, 0.08, 48, -5.6, -201.62, M.warning));
  const objectiveBaseY = 4.0, objectiveZ = -258.4;
  group.add(edgeBox(27, 7.4, 0.16, 48, objectiveBaseY + 3.7, objectiveZ, M.heroGlass, 0, 0.025));
  for (const x of [35, 42, 49, 56, 63]) group.add(box(0.18, 7.6, 0.22, x, objectiveBaseY + 3.8, objectiveZ + 0.04, M.loadingFrame));
  group.add(box(27.2, 0.22, 0.34, 48, objectiveBaseY + 7.6, objectiveZ, M.roof));
  group.add(box(27.2, 0.18, 0.3, 48, objectiveBaseY - 0.08, objectiveZ, M.concreteDark));
  for (const [x, material] of [[37, M.amber], [48, M.light], [59, M.red]]) interiorConsole(x, objectiveZ + 2.1, objectiveBaseY, Math.PI, material);
  for (const x of [39, 48, 57]) {
    group.add(box(3.2, 1.5, 0.08, x, objectiveBaseY + 5.1, objectiveZ + 0.5, M.glass));
    group.add(box(2.5, 0.72, 0.06, x, objectiveBaseY + 5.1, objectiveZ + 0.42, M.light));
  }
  group.add(box(0.16, 5.5, 0.16, 48, objectiveBaseY + 2.75, objectiveZ + 0.35, M.warning));
}
...
group.traverse(o => {
  if (!o.isMesh) return;
  if (o.userData.facilityFeature || o.parent?.userData?.facilityFeature) return;
...
for (const { m, geos } of byMat.values()) {
  ...
  for (const matching of compatible.values()) {
    ...
    triCount += mergedGeo.index ? mergedGeo.index.count / 3 : mergedGeo.attributes.position.count / 3;
    merged.add(new Mesh(mergedGeo, m));
  }
}
for (const { feature, group: featureGroup } of v24FeatureGroups) {
  ...
  merged.add(featureGroup);
}
for (const { feature, group: featureGroup } of v25OperationalDetailGroups) {
  featureGroup.updateMatrixWorld(true);
  featureGroup.traverse(o => {
    if (!o.isMesh) return;
    triCount += o.geometry.index ? o.geometry.index.count / 3 : o.geometry.attributes.position.count / 3;
...
if (directorMacroCycle2Only) merged.userData.directorMacroCycle2 = directorMacroCycle2Network;
if (directorMacroCycle3Only) merged.userData.directorMacroCycle3 = directorMacroCycle3Network;
if (directorMacroCycle4Only) merged.userData.directorMacroCycle4V22VisualCorrection = directorMacroCycle4V22VisualCorrection;
if (directorMacroCycle4V24Only) merged.userData.directorMacroCycle4V24VisualCorrection = directorMacroCycle4V24VisualCorrection;
if (directorMacroCycle4V25Only) merged.userData.directorMacroCycle4V25OperationalDetail = directorMacroCycle4V25OperationalDetail;
if (directorMacroCycle4V26Only) merged.userData.directorMacroCycle4V26VisualRecovery = directorMacroCycle4V26VisualRecovery;
...
if (directorMacroCycle4V26Only) {
  for (const [x, y, z, intensity, range] of [
    [-280, 7.4, 234, 2.2, 24], [-110, 5.8, 129, 1.6, 20], [-190, 5.8, 25, 1.8, 22],
    [-65, 4.8, 29, 1.5, 18], [300, 8.4, -66, 2.2, 26], [330, 11.8, 80, 2.6, 30],
    [330, 7.5, -139, 2.1, 24], [48, 8.8, -257, 3.8, 20],
  ]) {
    const light = new PointLight(0xffc486, intensity, range, 2);
    light.name = `director-v26-practical-${x}-${z}`;
...
const report = {
  ...
  sourceSha256,
  generatorSha256,
  deterministicProfile: { random: 'mulberry32', stableIdSeed: 'string-hash-31', input: sourceSha256, isolatedTransferOnly, campusSpineOnly, operationalStreetwallOnly, openCellNetworkOnly, openCellNetworkV2Only, openCellNetworkV3Only, directorMacroCycle2Only, directorMacroCycle3Only, directorMacroCycle4Only, directorMacroCycle4V24Only, directorMacroCycle4V25Only, directorMacroCycle4V26Only },
  ...
  directorMacroCycle2: directorMacroCycle2Only ? directorMacroCycle2Network : null,
  directorMacroCycle3: directorMacroCycle3Only ? directorMacroCycle3Network : null,
  directorMacroCycle4: directorMacroCycle4Only ? directorMacroCycle4Network : null,
  directorMacroCycle4V20VisualCorrection: directorMacroCycle4Only ? directorMacroCycle4V20VisualCorrection : null,
  directorMacroCycle4V22VisualCorrection: directorMacroCycle4Only ? directorMacroCycle4V22VisualCorrection : null,
  directorMacroCycle4V24VisualCorrection: directorMacroCycle4V24Only ? directorMacroCycle4V24VisualCorrection : null,
  directorMacroCycle4V25OperationalDetail: directorMacroCycle4V25Only ? directorMacroCycle4V25OperationalDetail : null,
  directorMacroCycle4V26VisualRecovery: directorMacroCycle4V26Only ? directorMacroCycle4V26VisualRecovery : null,
...
c('campus-spine: placement, support, and contact status recorded', !campusSpineOnly || campusSpine.features.every(feature => feature.placementStatus === 'PASS' && feature.supportStatus === 'PASS' && feature.contactStatus === 'PASS'));
  ...
  c('director-cycle-2: distributed features carry source/reference/contract/evidence trace', !directorMacroCycle2Only || (directorMacroCycle2Network.features.length === directorMacroCycle2Specs.length && directorMacroCycle2Network.features.every(feature => feature.sourceRefs.length >= 10 && feature.referenceIds.length >= 4 && feature.contractIds.includes('source-canonical') && feature.evidenceViews.length >= 6)));
  c('director-cycle-2: placement, support, contact, and clearance status recorded', !directorMacroCycle2Only || directorMacroCycle2Network.features.every(feature => feature.placementStatus === 'PASS' && feature.supportStatus === 'PASS' && feature.contactStatus === 'PASS' && feature.clearance.pad === 'PASS' && feature.clearance.minimumBuildingMargin >= 2 && feature.clearance.minimumRouteMargin >= 2 && feature.clearance.minimumAirLaneMargin >= 2 && feature.clearance.minimumGameplayMargin >= 2));
  c('director-cycle-2: paired facility relationships are complete', !directorMacroCycle2Only || directorMacroCycle2Network.features.every(feature => directorMacroCycle2Network.features.some(other => other.id === feature.pairedWith) && feature.relationship));
  c('director-cycle-2: distinct facility families are present', !directorMacroCycle2Only || ['warehouse', 'process', 'plant', 'core', 'admin', 'loading', 'boundary', 'service'].every(family => directorMacroCycle2Network.features.some(feature => feature.family === family)));
  c('director-cycle-2: triangle growth stays within 15 percent', !directorMacroCycle2Only || Math.round(triCount) <= directorMacroCycle2Network.compoundPolicy.maxTriangles, `${Math.round(triCount)} <= ${directorMacroCycle2Network.compoundPolicy.maxTriangles}`);
  c('director-cycle-3: connected features carry trace fields', !directorMacroCycle3Only || (directorMacroCycle3Network.features.length === directorMacroCycle3Specs.length && directorMacroCycle3Network.features.every(feature => feature.sourceRefs.length >= 14 && feature.referenceIds.includes('user-supplied-industrial-campus-board') && feature.contractIds.includes('connected-compound-massing') && feature.evidenceViews.length >= 12)));
  c('director-cycle-3: placement and contact status recorded', !directorMacroCycle3Only || directorMacroCycle3Network.features.every(feature => feature.placementStatus === 'PASS' && feature.supportStatus === 'PASS' && feature.contactStatus === 'PASS' && feature.clearance.pad === 'PASS' && feature.clearance.minimumBuildingMargin >= 2 && feature.clearance.minimumRouteMargin >= 2 && feature.clearance.minimumAirLaneMargin >= 2 && feature.clearance.minimumGameplayMargin >= 2));
  c('director-cycle-3: paired compound relationships are complete', !directorMacroCycle3Only || directorMacroCycle3Network.features.every(feature => directorMacroCycle3Network.features.some(other => other.id === feature.pairedWith) && feature.relationship));
  c('director-cycle-3: triangle growth stays within 15 percent', !directorMacroCycle3Only || Math.round(triCount) <= directorMacroCycle3Network.compoundPolicy.maxTriangles, `${Math.round(triCount)} <= ${directorMacroCycle3Network.compoundPolicy.maxTriangles}`);
  c('director-cycle-4: continuous substrate is present', !directorMacroCycle4Only || (directorMacroCycle4Network.strategy === 'continuous-hardstand-outboard-context-campus-cycle-4' && directorMacroCycle4Network.features.length === directorMacroCycle4Specs.length));
  c('director-cycle-4: context features carry trace fields', !directorMacroCycle4Only || directorMacroCycle4Network.features.every(feature => feature.sourceRefs.length >= 16 && feature.referenceIds.includes('wet-industrial-hardstand') && feature.contractIds.includes('continuous-campus-substrate') && feature.evidenceViews.length >= 6));
  c('director-cycle-4: placement, support, contact, and clearance status recorded', !directorMacroCycle4Only || directorMacroCycle4Network.features.every(feature => feature.placementStatus === 'PASS' && feature.supportStatus === 'PASS' && feature.contactStatus === 'PASS' && feature.clearance.canvas === 'PASS' && feature.clearance.minimumBuildingMargin >= 2 && feature.clearance.minimumRouteMargin >= 2 && feature.clearance.minimumAirLaneMargin >= 2));
  c('director-cycle-4: paired context relationships are complete', !directorMacroCycle4Only || directorMacroCycle4Network.features.every(feature => directorMacroCycle4Network.features.some(other => other.id === feature.pairedWith) && feature.relationship));
  c('director-cycle-4-v20: visual correction carries trace fields', !directorMacroCycle4Only || (directorMacroCycle4V20VisualCorrection.features.length === 11 && directorMacroCycle4V20VisualCorrection.sourceRefs.length >= 7 && directorMacroCycle4V20VisualCorrection.referenceIds.includes('user-supplied-industrial-campus-board') && directorMacroCycle4V20VisualCorrection.contractIds.includes('routes-unchanged') && directorMacroCycle4V20VisualCorrection.evidenceViews.length >= 8));
  c('director-cycle-4-v20: visual correction placement and contact recorded', !directorMacroCycle4Only || directorMacroCycle4V20VisualCorrection.features.every(feature => feature.placementStatus === 'PASS' && feature.supportStatus === 'PASS' && feature.contactStatus === 'PASS'));
  c('director-cycle-4-v22: connected service frontage carries trace fields', !directorMacroCycle4Only || (directorMacroCycle4V22VisualCorrection.features.length === 8 && directorMacroCycle4V22VisualCorrection.sourceRefs.length >= 8 && directorMacroCycle4V22VisualCorrection.referenceIds.includes('user-supplied-industrial-campus-board') && directorMacroCycle4V22VisualCorrection.contractIds.includes('routes-unchanged') && directorMacroCycle4V22VisualCorrection.evidenceViews.length >= 10));
  c('director-cycle-4-v22: connected service frontage placement and contact recorded', !directorMacroCycle4Only || directorMacroCycle4V22VisualCorrection.features.every(feature => feature.placementStatus === 'PASS' && feature.supportStatus === 'PASS' && feature.contactStatus === 'PASS'));
  c('director-cycle-4-v24: authored-pad massing carries trace fields', !directorMacroCycle4V24Only || (directorMacroCycle4V24VisualCorrection.features.length === 18 && directorMacroCycle4V24VisualCorrection.sourceRefs.length >= 8 && directorMacroCycle4V24VisualCorrection.referenceIds.includes('user-supplied-industrial-campus-board') && directorMacroCycle4V24VisualCorrection.contractIds.includes('routes-unchanged') && directorMacroCycle4V24VisualCorrection.evidenceViews.length >= 12 && directorMacroCycle4V24VisualCorrection.features.every(feature => feature.sourceRefs.length >= 4 && feature.routes.length > 0 && feature.padId)));
  c('director-cycle-4-v24: authored-pad placement status recorded', !directorMacroCycle4V24Only || directorMacroCycle4V24VisualCorrection.features.every(feature => feature.placementStatus === 'DECLARED' && feature.supportStatus === 'DECLARED' && feature.contactStatus === 'DECLARED'));
  c('director-cycle-4-v25: operational detail carries trace fields', !directorMacroCycle4V25Only || (directorMacroCycle4V25OperationalDetail.features.length === 10 && directorMacroCycle4V25OperationalDetail.sourceRefs.length >= 10 && directorMacroCycle4V25OperationalDetail.referenceIds.includes('user-supplied-industrial-campus-board') && directorMacroCycle4V25OperationalDetail.contractIds.includes('grounded-operational-detail') && directorMacroCycle4V25OperationalDetail.evidenceViews.length >= 15 && directorMacroCycle4V25OperationalDetail.features.every(feature => feature.sourceRefs.length >= 4 && feature.routes.length > 0 && feature.padId)));
  c('director-cycle-4-v25: operational detail placement recorded', !directorMacroCycle4V25Only || directorMacroCycle4V25OperationalDetail.features.every(feature => feature.placementStatus === 'PASS' && feature.supportStatus === 'PASS' && feature.contactStatus === 'PASS' && feature.clearance.minimumBuildingMargin >= 2 && feature.clearance.minimumRouteMargin >= 2 && feature.clearance.minimumAirLaneMargin >= 2 && feature.clearance.minimumGameplayMargin >= 2 && feature.clearance.minimumExistingFeatureMargin >= 2));
  c('director-cycle-4: triangle growth stays within 15 percent', !directorMacroCycle4Only || Math.round(triCount) <= directorMacroCycle4Network.compoundPolicy.maxTriangles, `${Math.round(triCount)} <= ${directorMacroCycle4Network.compoundPolicy.maxTriangles}`);

#!/usr/bin/env node
// Independent audit for the isolated macro-scale v3 open-cell candidate.
import fs from 'node:fs';
import { createHash } from 'node:crypto';
import { dirname } from 'node:path';

const sourcePath = process.argv[2] || 'blockout/blockout-full-v1.json';
const glbPath = process.argv[3] || '.context/ab/macro-open-cell-network-v3/facility-built-macro-open-cell-network-v3.glb';
const reportPath = process.argv[4] || '.context/ab/macro-open-cell-network-v3/build-report.json';
const outPath = process.argv[5] || '.context/ab/macro-open-cell-network-v3/macro-open-cell-network-v3-audit.json';
...
const expectedIds = [
  'ocn-v3-gate-main', 'ocn-v3-rear-warehouse', 'ocn-v3-west-warehouse', 'ocn-v3-courtyard-yard',
  'ocn-v3-core-ops', 'ocn-v3-bridge-north', 'ocn-v3-plant-south', 'ocn-v3-boundary-yard', 'ocn-v3-tunnel-portal',
];
...
const exported = document.nodes?.find(node => node.extras?.macroCampusNetworkV3)?.extras.macroCampusNetworkV3;
const candidate = report.macroCampusNetworkV3;
...
check('candidate-generation-profile-bound', report.sourceSha256 === expectedSourceSha256
  ...
  && report.deterministicProfile?.openCellNetworkV2Only === false
  && report.deterministicProfile?.openCellNetworkV3Only === true
...
check('macro-campus-exported', Boolean(exported));
check('macro-campus-report-export-match', JSON.stringify(exported) === JSON.stringify(candidate));
check('macro-campus-schema', candidate?.schemaVersion === 1 && candidate?.strategy === 'macro-open-cell-campus-network-v3' && candidate?.owner === 'recovery-cycle-7');
...
check('triangle-growth-cap', artifactIdentity.indexedTriangles <= baselineIdentity.indexedTriangles * 1.15 && artifactIdentity.indexedTriangles <= 881216, `${artifactIdentity.indexedTriangles} <= ${Math.floor(baselineIdentity.indexedTriangles * 1.15)} and 881216`);
...
console.log(`MACRO CAMPUS NETWORK V3 AUDIT: ${result.summary.passed}/${result.summary.checks} checks passed`);
